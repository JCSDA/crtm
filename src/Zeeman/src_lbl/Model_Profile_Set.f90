...
!
! PURPOSE:
!       Module to define model profiles
!
! CATEGORY:
!       Atmospheric Profile Sets
!
! LANGUAGE:
!       Fortran-90
!
...
!
! CONTAINS:
!       Load_Model_Profile:  Function to load the requested model atmosphere
!                            into the shared data arrays.
!
! INCLUDE FILES:
!       None.
!
...
MODULE Model_Profile_Set
  ...
  INTEGER, PUBLIC, PARAMETER :: N_MODELS          = 6
  INTEGER, PUBLIC, PARAMETER :: N_MODEL_LEVELS    = 50
  INTEGER, PUBLIC, PARAMETER :: N_MODEL_LAYERS    = N_MODEL_LEVELS - 1
  INTEGER, PUBLIC, PARAMETER :: N_MODEL_ABSORBERS = 28

  ! -- Model description
  CHARACTER( * ), PUBLIC, PARAMETER, DIMENSION( N_MODELS ) :: MODEL_PROFILE_NAME = &
    (/ 'Tropical          ', &
       'Midlatitude summer', &
       'Midlatitude winter', &
       'Subarctic summer  ', &
  ...
  ! -- Absorber names
  CHARACTER( * ), PUBLIC, PARAMETER, DIMENSION( N_MODEL_ABSORBERS ) :: MODEL_ABSORBER_NAME = &
    (/ 'H2O     ', 'CO2     ', 'O3      ', 'N2O     ', &
       'CO      ', 'CH4     ', 'O2      ', 'NO      ', &
       'SO2     ', 'NO2     ', 'NH3     ', 'HNO3    ', &
       'OH      ', 'HF      ', 'HCL     ', 'HBR     ', &
  ...
  ! -- Absorber units (for LBLRTM)
  INTEGER,        PUBLIC, PARAMETER :: MODEL_ABSORBER_UNITS_ID   = 2
  CHARACTER( * ), PUBLIC, PARAMETER :: MODEL_ABSORBER_UNITS_NAME = 'ppmv'
  CHARACTER( * ), PUBLIC, PARAMETER :: MODEL_ABSORBER_UNITS_FLAG = 'A'

  ! -- Model latitudes (Northern hemisphere annual averages)
  REAL( fp_kind ), PUBLIC, PARAMETER, DIMENSION( N_MODELS ) :: MODEL_LATITUDE = &
  ...
  ! -------------------------
  ! PUBLIC Module shared data
  ! -------------------------

  REAL( fp_kind ), PUBLIC, SAVE, DIMENSION( N_MODEL_LEVELS )      :: model_level_altitude    = ZERO
  REAL( fp_kind ), PUBLIC, SAVE, DIMENSION( N_MODEL_LEVELS )      :: model_level_pressure    = ZERO
  REAL( fp_kind ), PUBLIC, SAVE, DIMENSION( N_MODEL_LEVELS )      :: model_level_temperature = ZERO
  REAL( fp_kind ), PUBLIC, SAVE, DIMENSION( N_MODEL_LEVELS )      :: model_level_density     = ZERO
  REAL( fp_kind ), PUBLIC, SAVE, &
                   DIMENSION( N_MODEL_LEVELS, N_MODEL_ABSORBERS ) :: model_level_absorber    = ZERO
...
!
! PURPOSE:
!       Function to fill the module shared data arrays with the requested
!       climatology model profile.
!
!       These data were taken directly from the MLATMB BLOCK DATA subprogram
!       of LBLRTM.
!
! CATEGORY:
!       Atmospheric Profile Sets
...
!
! CALLING SEQUENCE:
!       result = Load_Model_Profile( profile, &  ! Input
!
...
!                           model climatology set. Valid values are:
!                             1: Tropical
...
!                 = FAILURE an invalid profile number was specified
!
! CALLS:
...
!
! PROCEDURE:
!       If a valid profile number is passed, the public module shared data
!       arrays are loaded with the requested profile's data:
!
!         model_level_altitude:    Model level altitudes (same for all models)
!                                  UNITS:      kilometres (km)
!                                  TYPE:       REAL( fp_kind )
!                                  DIMENSION:  Rank-1, N_MODEL_LEVELS
!                                  ATTRIBUTES: PUBLIC, SAVE
!
!         model_level_pressure:    Model level pressures
!                                  UNITS:      hectoPascals (hPa)
!                                  TYPE:       REAL( fp_kind )
!                                  DIMENSION:  Rank-1, N_MODEL_LEVELS
!                                  ATTRIBUTES: PUBLIC, SAVE
!
!         model_level_temperature: Model level temperatures
!                                  UNITS:      Kelvin (K)
!                                  TYPE:       REAL( fp_kind )
!                                  DIMENSION:  Rank-1, N_MODEL_LEVELS
!                                  ATTRIBUTES: PUBLIC, SAVE
!
!         model_level_density:     Model total air density.
!                                  UNITS:      molecules.cm^-3
!                                  TYPE:       REAL( fp_kind )
!                                  DIMENSION:  Rank-1, N_MODEL_LEVELS
!                                  ATTRIBUTES: PUBLIC, SAVE
!
!         model_level_absorber:    Model absorber concentrations. The molecular
!                                  species for which data is available are:
...
!                                  Only the first SIX molecular species are
!                                  distinct between models. The remainder are
!                                  default values that are the same for every
!                                  model atmosphere.
!                                  UNITS:      ppmv
!                                  TYPE:       REAL( fp_kind )
!                                  DIMENSION:  Rank-2, N_MODEL_LEVELS x N_MODEL_ABSORBERS
!                                  ATTRIBUTES: PUBLIC, SAVE
!
! CREATION HISTORY:
!       Written by:     Paul van Delst, CIMSS/SSEC 13-Nov-2001
...
!--------------------------------------------------------------------------------
  ...
  FUNCTION Load_Model_Profile ( profile,      &  ! Input
    ...
    IF ( profile < 1 .OR. profile > N_MODELS ) THEN
    ...
    !#--------------------------------------------------------------------------#
    !#          -- FILL THE ALTITUDE ARRAY - SAME FOR ALL MODELS --             #
    !#--------------------------------------------------------------------------#

    model_level_altitude = (/ &
        0.0_fp_kind,   1.0_fp_kind,   2.0_fp_kind,   3.0_fp_kind,   4.0_fp_kind, &
        5.0_fp_kind,   6.0_fp_kind,   7.0_fp_kind,   8.0_fp_kind,   9.0_fp_kind, &
       10.0_fp_kind,  11.0_fp_kind,  12.0_fp_kind,  13.0_fp_kind,  14.0_fp_kind, &
       15.0_fp_kind,  16.0_fp_kind,  17.0_fp_kind,  18.0_fp_kind,  19.0_fp_kind, &
       20.0_fp_kind,  21.0_fp_kind,  22.0_fp_kind,  23.0_fp_kind,  24.0_fp_kind, &
       25.0_fp_kind,  27.5_fp_kind,  30.0_fp_kind,  32.5_fp_kind,  35.0_fp_kind, &
       37.5_fp_kind,  40.0_fp_kind,  42.5_fp_kind,  45.0_fp_kind,  47.5_fp_kind, &
       50.0_fp_kind,  55.0_fp_kind,  60.0_fp_kind,  65.0_fp_kind,  70.0_fp_kind, &
       75.0_fp_kind,  80.0_fp_kind,  85.0_fp_kind,  90.0_fp_kind,  95.0_fp_kind, &
      100.0_fp_kind, 105.0_fp_kind, 110.0_fp_kind, 115.0_fp_kind, 120.0_fp_kind /)
    ...
    SELECT CASE ( profile )
      ...
      CASE ( 1 )
        ...
        ! -- Total air density
        model_level_density = (/ &
          2.450e+19_fp_kind, 2.231e+19_fp_kind, 2.028e+19_fp_kind, 1.827e+19_fp_kind, 1.656e+19_fp_kind, &
          1.499e+19_fp_kind, 1.353e+19_fp_kind, 1.218e+19_fp_kind, 1.095e+19_fp_kind, 9.789e+18_fp_kind, &
          8.747e+18_fp_kind, 7.780e+18_fp_kind, 6.904e+18_fp_kind, 6.079e+18_fp_kind, 5.377e+18_fp_kind, &
          4.697e+18_fp_kind, 4.084e+18_fp_kind, 3.486e+18_fp_kind, 2.877e+18_fp_kind, 2.381e+18_fp_kind, &
          1.981e+18_fp_kind, 1.651e+18_fp_kind, 1.381e+18_fp_kind, 1.169e+18_fp_kind, 9.920e+17_fp_kind, &
          8.413e+17_fp_kind, 5.629e+17_fp_kind, 3.807e+17_fp_kind, 2.598e+17_fp_kind, 1.789e+17_fp_kind, &
          1.243e+17_fp_kind, 8.703e+16_fp_kind, 6.147e+16_fp_kind, 4.352e+16_fp_kind, 3.119e+16_fp_kind, &
          2.291e+16_fp_kind, 1.255e+16_fp_kind, 6.844e+15_fp_kind, 3.716e+15_fp_kind, 1.920e+15_fp_kind, &
          9.338e+14_fp_kind, 4.314e+14_fp_kind, 1.801e+14_fp_kind, 7.043e+13_fp_kind, 2.706e+13_fp_kind, &
          1.098e+13_fp_kind, 4.445e+12_fp_kind, 1.941e+12_fp_kind, 8.706e+11_fp_kind, 4.225e+11_fp_kind /)


        ! -- Major absorber #1, H2O
        model_level_absorber( :, 1 ) = (/ &
      ...
      CASE ( 2 )
        ...
        ! -- Total air density
        model_level_density = (/ &
          2.496e+19_fp_kind, 2.257e+19_fp_kind, 2.038e+19_fp_kind, 1.843e+19_fp_kind, 1.666e+19_fp_kind, &
          1.503e+19_fp_kind, 1.351e+19_fp_kind, 1.212e+19_fp_kind, 1.086e+19_fp_kind, 9.716e+18_fp_kind, &
          8.656e+18_fp_kind, 7.698e+18_fp_kind, 6.814e+18_fp_kind, 6.012e+18_fp_kind, 5.141e+18_fp_kind, &
          4.368e+18_fp_kind, 3.730e+18_fp_kind, 3.192e+18_fp_kind, 2.715e+18_fp_kind, 2.312e+18_fp_kind, &
          1.967e+18_fp_kind, 1.677e+18_fp_kind, 1.429e+18_fp_kind, 1.223e+18_fp_kind, 1.042e+18_fp_kind, &
          8.919e+17_fp_kind, 6.050e+17_fp_kind, 4.094e+17_fp_kind, 2.820e+17_fp_kind, 1.927e+17_fp_kind, &
          1.338e+17_fp_kind, 9.373e+16_fp_kind, 6.624e+16_fp_kind, 4.726e+16_fp_kind, 3.398e+16_fp_kind, &
          2.500e+16_fp_kind, 1.386e+16_fp_kind, 7.668e+15_fp_kind, 4.196e+15_fp_kind, 2.227e+15_fp_kind, &
          1.109e+15_fp_kind, 4.996e+14_fp_kind, 1.967e+14_fp_kind, 7.204e+13_fp_kind, 2.541e+13_fp_kind, &
          9.816e+12_fp_kind, 3.816e+12_fp_kind, 1.688e+12_fp_kind, 8.145e+11_fp_kind, 4.330e+11_fp_kind /)
      ...
      CASE ( 3 )
        ...
        ! -- Total air density
        model_level_density = (/ &
          2.711e+19_fp_kind, 2.420e+19_fp_kind, 2.158e+19_fp_kind, 1.922e+19_fp_kind, 1.724e+19_fp_kind, &
          1.542e+19_fp_kind, 1.376e+19_fp_kind, 1.225e+19_fp_kind, 1.086e+19_fp_kind, 9.612e+18_fp_kind, &
          8.472e+18_fp_kind, 7.271e+18_fp_kind, 6.237e+18_fp_kind, 5.351e+18_fp_kind, 4.588e+18_fp_kind, &
          3.931e+18_fp_kind, 3.368e+18_fp_kind, 2.886e+18_fp_kind, 2.473e+18_fp_kind, 2.115e+18_fp_kind, &
          1.809e+18_fp_kind, 1.543e+18_fp_kind, 1.317e+18_fp_kind, 1.125e+18_fp_kind, 9.633e+17_fp_kind, &
          8.218e+17_fp_kind, 5.536e+17_fp_kind, 3.701e+17_fp_kind, 2.486e+17_fp_kind, 1.647e+17_fp_kind, &
          1.108e+17_fp_kind, 7.540e+16_fp_kind, 5.202e+16_fp_kind, 3.617e+16_fp_kind, 2.570e+16_fp_kind, &
          1.863e+16_fp_kind, 1.007e+16_fp_kind, 5.433e+15_fp_kind, 2.858e+15_fp_kind, 1.477e+15_fp_kind, &
          7.301e+14_fp_kind, 3.553e+14_fp_kind, 1.654e+14_fp_kind, 7.194e+13_fp_kind, 3.052e+13_fp_kind, &
          1.351e+13_fp_kind, 6.114e+12_fp_kind, 2.952e+12_fp_kind, 1.479e+12_fp_kind, 7.836e+11_fp_kind /)
      ...
      CASE ( 4 )
        ...
        ! -- Total air density
        model_level_density = (/ &
          2.549e+19_fp_kind, 2.305e+19_fp_kind, 2.080e+19_fp_kind, 1.873e+19_fp_kind, 1.682e+19_fp_kind, &
          1.508e+19_fp_kind, 1.357e+19_fp_kind, 1.216e+19_fp_kind, 1.088e+19_fp_kind, 9.701e+18_fp_kind, &
          8.616e+18_fp_kind, 7.402e+18_fp_kind, 6.363e+18_fp_kind, 5.471e+18_fp_kind, 4.699e+18_fp_kind, &
          4.055e+18_fp_kind, 3.476e+18_fp_kind, 2.987e+18_fp_kind, 2.568e+18_fp_kind, 2.208e+18_fp_kind, &
          1.899e+18_fp_kind, 1.632e+18_fp_kind, 1.403e+18_fp_kind, 1.207e+18_fp_kind, 1.033e+18_fp_kind, &
          8.834e+17_fp_kind, 6.034e+17_fp_kind, 4.131e+17_fp_kind, 2.839e+17_fp_kind, 1.938e+17_fp_kind, &
          1.344e+17_fp_kind, 9.402e+16_fp_kind, 6.670e+16_fp_kind, 4.821e+16_fp_kind, 3.516e+16_fp_kind, &
          2.581e+16_fp_kind, 1.421e+16_fp_kind, 7.946e+15_fp_kind, 4.445e+15_fp_kind, 2.376e+15_fp_kind, &
          1.198e+15_fp_kind, 5.311e+14_fp_kind, 2.022e+14_fp_kind, 7.221e+13_fp_kind, 2.484e+13_fp_kind, &
          9.441e+12_fp_kind, 3.624e+12_fp_kind, 1.610e+12_fp_kind, 7.951e+11_fp_kind, 4.311e+11_fp_kind /)
      ...
      CASE ( 5 )
        ...
        ! -- Total air density
        model_level_density = (/ &
          2.855e+19_fp_kind, 2.484e+19_fp_kind, 2.202e+19_fp_kind, 1.950e+19_fp_kind, 1.736e+19_fp_kind, &
          1.552e+19_fp_kind, 1.383e+19_fp_kind, 1.229e+19_fp_kind, 1.087e+19_fp_kind, 9.440e+18_fp_kind, &
          8.069e+18_fp_kind, 6.898e+18_fp_kind, 5.893e+18_fp_kind, 5.039e+18_fp_kind, 4.308e+18_fp_kind, &
          3.681e+18_fp_kind, 3.156e+18_fp_kind, 2.704e+18_fp_kind, 2.316e+18_fp_kind, 1.982e+18_fp_kind, &
          1.697e+18_fp_kind, 1.451e+18_fp_kind, 1.241e+18_fp_kind, 1.061e+18_fp_kind, 9.065e+17_fp_kind, &
          7.742e+17_fp_kind, 5.134e+17_fp_kind, 3.423e+17_fp_kind, 2.292e+17_fp_kind, 1.533e+17_fp_kind, &
          1.025e+17_fp_kind, 6.927e+16_fp_kind, 4.726e+16_fp_kind, 3.266e+16_fp_kind, 2.261e+16_fp_kind, &
          1.599e+16_fp_kind, 8.364e+15_fp_kind, 4.478e+15_fp_kind, 2.305e+15_fp_kind, 1.181e+15_fp_kind, &
          6.176e+14_fp_kind, 3.127e+14_fp_kind, 1.531e+14_fp_kind, 7.244e+13_fp_kind, 3.116e+13_fp_kind, &
          1.403e+13_fp_kind, 6.412e+12_fp_kind, 3.099e+12_fp_kind, 1.507e+12_fp_kind, 7.814e+11_fp_kind /)
      ...
      CASE ( 6 )
        ...
        ! -- Total air density
        model_level_density = (/ &
          2.548e+19_fp_kind, 2.313e+19_fp_kind, 2.094e+19_fp_kind, 1.891e+19_fp_kind, 1.704e+19_fp_kind, &
          1.532e+19_fp_kind, 1.373e+19_fp_kind, 1.228e+19_fp_kind, 1.094e+19_fp_kind, 9.719e+18_fp_kind, &
          8.602e+18_fp_kind, 7.589e+18_fp_kind, 6.489e+18_fp_kind, 5.546e+18_fp_kind, 4.739e+18_fp_kind, &
          4.050e+18_fp_kind, 3.462e+18_fp_kind, 2.960e+18_fp_kind, 2.530e+18_fp_kind, 2.163e+18_fp_kind, &
          1.849e+18_fp_kind, 1.575e+18_fp_kind, 1.342e+18_fp_kind, 1.144e+18_fp_kind, 9.765e+17_fp_kind, &
          8.337e+17_fp_kind, 5.640e+17_fp_kind, 3.830e+17_fp_kind, 2.524e+17_fp_kind, 1.761e+17_fp_kind, &
          1.238e+17_fp_kind, 8.310e+16_fp_kind, 5.803e+16_fp_kind, 4.090e+16_fp_kind, 2.920e+16_fp_kind, &
          2.136e+16_fp_kind, 1.181e+16_fp_kind, 6.426e+15_fp_kind, 3.386e+15_fp_kind, 1.723e+15_fp_kind, &
          8.347e+14_fp_kind, 3.832e+14_fp_kind, 1.711e+14_fp_kind, 7.136e+13_fp_kind, 2.924e+13_fp_kind, &
          1.189e+13_fp_kind, 5.033e+12_fp_kind, 2.144e+12_fp_kind, 9.688e+11_fp_kind, 5.114e+11_fp_kind /)
...
! Revision 1.1  2002/07/19 16:33:52  paulv
! Initial checkin.
!
!
!
