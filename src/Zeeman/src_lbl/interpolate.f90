...
!
! CALLING SEQUENCE:
!       result = polynomial_interpolate( x,                        &  ! Input
!                                        y,                        &  ! Input
!                                        x_int,                    &  ! Input
!                                        y_int,                    &  ! Output
...
!                     TYPE:       REAL( Single ) or REAL( Double )
...
!                     TYPE:       Single- or Double-precision float
!                     DIMENSION:  Scalar or Rank-1 array
!                     ATTRIBUTES: INTENT( IN )
!
!
...
!
! FUNCTION RESULT:
!       The function returns an integer error flag defined in the ERROR_HANDLER
!       module.
!       If result == SUCCESS, function executed normally.
!                 == FAILURE, error occurred in function.
!
! CALLS:
!       bisection_search:  Function to search an array using the bisection method.
!                          This function is an adaptation from Numerical Recipes.
!
!       value_locate:      Function that finds the intervals within a given monotonic
...
!       - All floating point inputs and outputs must be either single
!         OR double precision, not mixtures of both.
!       - All arguments must be typed in the calling routine to allow
!         resolution of the generic to specific function.
!
! EXAMPLE:
!
!       USE error_handler
!       USE interpolate
!
!       ! -- Define array sizes and boundary limits
!       INTEGER, PARAMETER :: N  = 100
!       REAL,    PARAMETER :: X1 = -10.0, X2 = 10.0
!       INTEGER, PARAMETER :: N_INT  = 1000
!       REAL,    PARAMETER :: X1_INT = -2.0, X2_INT = 3.0
!
!       ! -- Local variable defintions
!       INTEGER :: j, error_status
!       REAL, DIMENSION( N )     :: x, y
!       REAL, DIMENSION( N_INT ) :: x_int, y_int
!
!       ! -- Create x-vector
!       x = (/ ( REAL( j-1 ), j = 1, N ) /)
!       x = x * ( X2 - X1 ) + X1
...
!       y = x**2 - x + 1.0
!
!       ! -- Create vector at which interpolates are desired
!       x_int = (/ ( REAL( j-1 ), j = 1, N_INT ) /)
!       x_int = x_int * ( X2_INT - X1_INT ) + X1_INT
!
!       ! -- cubic interpolation
!       error_status = cubic_interpolate( x, y, x_int, y_int )
...
!
! CALLING SEQUENCE:
!       result = spline_initialize( x,                        &  ! Input
!                                   y,                        &  ! Input
!                                   y2,                       &  ! Output
!                                   RCS_Id      = RCS_Id,     &  ! Optional output
!                                   message_log = message_log )  ! Error messaging
...
!------------------------------------------------------------------------------
  ...
  FUNCTION splinit_double( x,            &  ! Input
    ...
    i_decomposition_loop : DO i = 2, n - 1
      ...
      u( i ) = ( ( numerator / denominator ) - offset ) / p
!if ( abs(u(i)) < epsilon(D_ONE) ) u(i)=D_ZERO
...
!
! CALLING SEQUENCE:
!       result = spline_interpolate( x,                        &  ! Input
!                                    y,                        &  ! Input
!                                    x_int,                    &  ! Input
!                                    y_int,                    &  ! Output
...
!
! CALLS:
!       spline_initialize:  Function to calculate the values of the second
!                           derivative of the interpolating function at the
!                           points defined by X
!
!       display_message:    Subroutine to output messages
!                           SOURCE: ERROR_HANDLER module
...
!
! RESTRICTIONS:
!       - Input X,Y data pairs must in ascending X order
