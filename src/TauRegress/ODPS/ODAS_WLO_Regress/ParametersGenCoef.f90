...
!  real(fp_kind),parameter :: DTAU_THRESHOLD = 0.001_fp_kind 
  ...
  real(fp_kind),parameter :: VirtEmiss_MW         = 0.0_fp_kind  ! virtial emissivity
!  real(fp_kind),parameter :: VirtEmiss_MW         = 0.80_fp_kind  ! virtial emissivity
...
!                                     4._fp_kind  /)    ! ozone !less stable
  ...
  integer      ,parameter :: Natmpred_max       = 17    ! # of atmospheric predictors for wlo

  integer      ,parameter :: Natmpred_maxused   = 6     ! # of atmos. predictors maxmumly used in regression equation
! integer      ,parMICROWAVE_SENSORameter :: Natmpred_maxused   = 2     ! (experiment)
! integer      ,parameter :: Natmpred_maxused   = 3     ! (experiment)
  ...
  integer,parameter   :: IdMolecule_All     =  10   ! total tau ID in netCDF file (total)
  integer,parameter   :: IdMolecule_Dry     =  20   ! dry tau component ID in netCDF file
  integer             :: IdMolecule_Wet             ! variable holding wet tau component ID
  integer,parameter   :: IdMolecule_Wet_IR  = 101   ! wet tau (line only) ID in netCDF file
  integer,parameter   :: IdMolecule_Wet_MW  =  12   ! wet tau ID for MW in netCDF file
  integer,parameter   :: IdMolecule_Ozo     = 114   ! ozone tau ID in netCDF file

  ! Gas IDs used internally: 1 - dry; 2 - wet; 3 - ozo
  ! This arrary converts internal gas Id to those used in line-by-line program
  
  integer :: gasID_convert(3)
  ...
  real(fp_kind),allocatable :: Geometric_Secant(:, :, :)        ! secant incidence angles for different layers
