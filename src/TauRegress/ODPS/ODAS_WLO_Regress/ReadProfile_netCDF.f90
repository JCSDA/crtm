...
module ReadProfile_netCDF
 ...
 contains
  ...
  subroutine Open_TransProf()
    ...
    integer                   :: Iflag
    integer                   :: i, m
    real(fp_kind),allocatable :: Geometric_angle(:,:,:)
...
!    print *, 'Platform name      : ', trim( platform_name )
    ...
    !----reading in Geometric angles
    allocate( Geometric_Angle (Nlay, n_angles, Natm) )
    allocate( Geometric_Secant(Nlay, n_angles, Natm) )
    IF(SpcCoeff%Sensor_Type == MICROWAVE_SENSOR)THEN
      DO i = 1, n_angles
      DO m = 1, Natm
        Geometric_Secant(:, i, m) = secant(i)
      END DO
      END DO
    ELSE
      Iflag = Read_GeometricAngle_netCDF( InFileName_TransProf, &  ! Input
                                          Geometric_Angle )! Output
    
      if( Iflag /= SUCCESS )then
        print *, '### ERROR IN GETTING Geometric Angles ###'
        print *, 'File Name : ', InFileName_TransProf
        print *, 'Error flg : ', Iflag
        stop 90
      endif
      Geometric_Secant = ONE/cos(Geometric_Angle * DEGREES_TO_RADIANS )
    END IF
    deallocate(Geometric_Angle)
  ...
  subroutine Read_TransProf( Ichan, &
                             alltrans_lev, &
                             drytrans_lev, wettrans_lev, ozntrans_lev, efctrans_lev )
    ...
    real(fp_kind),intent(out) :: efctrans_lev(0:Nlay,Nangle,Natm)   ! ozone
...
!!*** Dry and Ozone will not be used
!!      Iflag = Read_TauProfile_netCDF( InFileName_TransProf,&  ! Input
!!                                      idx_chan,            &  ! Input
!!                                      secant(Iang),        &  ! Input
!!                                      Iatm,                &  ! Input
...
!!      endif
      IF(SpcCoeff%Sensor_Type == MICROWAVE_SENSOR)THEN
        IdMolecule_Wet = IdMolecule_Wet_MW
      ELSE
        IdMolecule_Wet = IdMolecule_Wet_IR
      END IF
      gasID_convert = (/IdMolecule_Dry,IdMolecule_Wet,IdMolecule_Ozo /)
...
! Generate complimentary tau component for wet tau
        IF(wettrans_lev(ilev,Iang,Iatm) > ZERO)THEN
          efctrans_lev(ilev,Iang,Iatm) = alltrans_lev(ilev,Iang,Iatm) / wettrans_lev(ilev,Iang,Iatm)
        ELSE
          efctrans_lev(ilev,Iang,Iatm) = ZERO
        END IF
...
!! The following checks are removed since there are not dry and ozo components
IF( 1 == 0 )THEN
      if(ALL(SenIdWmo /= INTERFEROMETER_LIST)) THEN
