...
!            trans_lev(Ilev_1) >= POINT1 )cycle
      if( ( trans_lev(Ilev_1) - trans_lev(Ilev) ) < 10._fp_kind * TOLERANCE ) then
        Ilev_1 = Ilev
        cycle
      endif
...
!      print *, 'trans_lev:',trans_lev(Ilev) / trans_lev(Ilev_1) , log(trans_lev    (Ilev) / trans_lev    (Ilev_1) )
      
      predictand_lay(Ilev) = &
             log( - log( trans_lev    (Ilev) / trans_lev    (Ilev_1) ) &
                     / ( absamount_lev(Ilev) - absamount_lev(Ilev_1) ) )

!      print *, 'predictand_lay:', predictand_lay(Ilev)
      !--- update Ilev_1
  ...
  subroutine Get_AtmosPredictor( p_lay, t_lay, q_lay, o3_lay, &
    ...
    real(fp_kind),intent(in)   :: Geometric_Secant(Nlay,Nangle,Natm)             ! absorber amounts on levels
