...
MODULE Ocean_Permittivity
  ...
  TYPE :: GuillouVariables_type
    PRIVATE
    REAL(fp) :: t, s
    REAL(fp) :: f, f2, f0, f2po             ! Frequency terms
    REAL(fp) :: a1(0:N_A1), a2(0:N_A2), es
    REAL(fp) :: b1(0:N_B1), einf            ! Element 0 of b1 == einf
    REAL(fp) :: c1(0:N_C1), c2(0:N_C2)
    REAL(fp) :: d1(0:N_D1), d2(0:N_D2)
  ...
  TYPE :: EllisonVariables_type
    PRIVATE
    REAL(fp) :: t               ! Temperature in degC
    REAL(fp) :: f, f2           ! Frequency terms
    REAL(fp) :: tau1, tau2      ! Relaxation frequencies
    REAL(fp) :: delta1, delta2  ! Delta terms
    REAL(fp) :: d1, d2          ! Denominator terms
