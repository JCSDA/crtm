...
!
! PURPOSE:
!       Module of parameter definitions for the prototype CRTM (pCRTM).
!
! CATEGORY:
!       pCRTM
!
...
!
! CONTAINS:
!       Set_Max_n_Channels:    Subroutine to set the protected variable
!                              MAX_N_CHANNELS value in the Parameters
!                              module. This should *only* be done during
!                              the pCRTM initialisation.
!
!       Reset_Max_n_Channels:  Subroutine to reset the protected variable
!                              MAX_N_CHANNELS value in the Parameters
!                              module to an invalid value. This should
!                              *only* be done during the pCRTM destruction.
!
!       Get_Max_n_Channels:    Subroutine to GET the protected variable
...
!
! SIDE EFFECTS:
!       The MAX_N_CHANNELS protected variable can be modified.
!
! CREATION HISTORY:
...
MODULE Parameters
  ...
  !#----------------------------------------------------------------------------#
  !#                     -- DEFINE THE DIMENSION PARAMETERS --                  #
  !#----------------------------------------------------------------------------#

  ! ---------------------------
  ! Number of allowed absorbers
  ! ---------------------------
  ...
  ! -------------------------------------
  ! Number of predictors - total and used
  ! -------------------------------------
  ...
  !
  ! Because the number of channels that can be used is
  ! determined during the model initialisation, it is a
  ! "pseudo-parameter" in that it can be set, reset, or
  ! retrieved via the MAX_N_CHANNELS methods
  ! --------------------------------------------------------
  ...
  ! --------------------------------------------------
  ! The maximum number of profiles and layers accepted
  ! These values are arbitrary. Nothing magical.
  ! --------------------------------------------------
  ...
  !#----------------------------------------------------------------------------#
  !#            -- DIRECTION FLAGS FOR TRANSMITTANCE CALCULATIONS --            #
  !#----------------------------------------------------------------------------#
...
!  REAL( fp_kind ), PUBLIC, PARAMETER :: LIMIT_LOG = 4.5e+15_fp_kind   ! EXP( LIMIT_EXP )
  ...
  ! angle secant used in generating the gas absorption model coefficients,
  ...
  ! by the maximum angle secant used in generating the gas absorption
...
!       Parameters module. This should *only* be done during the pCRTM
!       initialisation.
!
! CATEGORY:
!       pCRTM
...
!       Parameters module to an invalid value. This should *only* be done
!       during the pCRTM destruction.
!
! CATEGORY:
!       pCRTM
!
...
!
! CALLING SEQUENCE:
!       CALL Get_Max_n_Channels( Value, Is_Set )
!
! INPUT ARGUMENTS:
!       None.
!
...
!-------------------------------------------------------------------------------
!
! $Id: parameters.f90,v 1.16 2004/12/22 17:43:33 paulv Exp $
!
...
! Revision 1.16  2004/12/22 17:43:33  paulv
! - Updated header documentation.
!
! Revision 1.15  2004/10/27 17:32:30  paulv
! - Added the parameter MINIMUM_ABSORBER_AMOUNT. This parameter is set to a
!   value such that 1/MINIMUM_ABSORBER_AMOUNT**4 is still a valid number, i.e.
!   greater than the output of the TINY() intrinsic.
! - Tidied up the module. Added some more documentation.
!
! Revision 1.14  2004/06/10 19:14:24  paulv
! - Altered the exponential and logarithmic tolerance limits based on
!   tests rather than precision.
!
