...
!
! PURPOSE:
!       Module containing routines to compute the integrated absorber
!       profiles for the prototype CRTM (pCRTM).
!
! CATEGORY:
!       pCRTM
!
...
!
! CONTAINS:
!       Compute_Absorber_Amount:     Subroutine to compute the integrated
...
!                                      - Ozone
!
!       Compute_Absorber_Amount_TL:  Subroutine to compute the tangent-
!                                    linear form of the integrated absorber 
!                                    profiles.
!
!       Compute_Absorber_Amount_AD:  Subroutine to compute the adjoint of
...
!
! CREATION HISTORY:
!       Written by:     Paul van Delst, CIMSS/SSEC 01-Aug-2000
...
!       amount for the wet, dry, and ozone absorbers through k atmospheric
!       layers.
!
...
!
! PURPOSE:
!       Subroutine to compute the tangent linear form of the integrated
...
!
! CALLING SEQUENCE:
!       CALL Compute_Absorber_Amount_TL( Pressure,       &  ! Input, K
!                                        Water_Vapor,    &  ! Input, K
!                                        Ozone,          &  ! Input, K
!                                        Pressure_TL,    &  ! Input, K
...
!                        UNITS:      hPa
!                        TYPE:       REAL( fp_kind )
!                        DIMENSION:  Rank-1 (K), number of levels - 1
!                        ATTRIBUTES: INTENT( IN )
!
...
!--------------------------------------------------------------------------------
  ...
  SUBROUTINE Compute_Absorber_Amount_TL( Pressure,       &  ! Input,  K
                                         Water_Vapor,    &  ! Input,  K
                                         Ozone,          &  ! Input,  K
                                         Pressure_TL,    &  ! Input,  K
                                         Water_Vapor_TL, &  ! Input,  K
...
!
! PURPOSE:
!       Subroutine to compute the adjoint of the integrated
!       profiles for all the absorbers. Currently the number of absorbers
!       are:
!         - Water vapor
!         - Dry/fixed gases (pressure == absorber amount)
...
!
! CALLING SEQUENCE:
!       CALL Compute_Absorber_Amount_AD( Pressure,       &  ! Input, K
!                                        Water_Vapor,    &  ! Input, K
!                                        Ozone,          &  ! Input, K
...
!--------------------------------------------------------------------------------
  ...
  SUBROUTINE  Compute_Absorber_Amount_AD( Pressure,       &  ! Input, K
                                          Water_Vapor,    &  ! Input, K
                                          Ozone,          &  ! Input, K
                                          Absorber_AD,    &  ! In/Output, 0:K x J
                                          Pressure_AD,    &  ! In/Output, K
...
!-------------------------------------------------------------------------------
!
! $Id: absorber_profile.f90,v 2.1 2004/12/22 17:43:32 paulv Exp $
!
! $Date: 2004/12/22 17:43:32 $
!
! $Revision: 2.1 $
!
! $Name:  $
...
! Revision 2.1  2004/12/22 17:43:32  paulv
