...
!       gas absorption calculation in the prototype CRTM (pCRTM).
!
! CATEGORY:
!       pCRTM
!
...
!
! CONTAINS:
!       Compute_Predictors:     Subroutine to calculate the predictor
!                               profiles.
!
!       Compute_Predictors_TL:  Subroutine to calculate the tangent-linear
!                               predictor profiles.
!
!       Compute_Predictors_AD:  Subroutine to calculate the adjoint
!                               predictor profiles.
!
! EXTERNALS:
!       None
...
!
! PURPOSE:
!       Subroutine to calculate the predictors for the gas absorption model.
!
! CATEGORY:
!       pCRTM
!
...
!--------------------------------------------------------------------------------
  ...
  SUBROUTINE Compute_Predictors ( Pressure,    &  ! Input,  K
    ...
    IF ( .NOT. PRESENT( No_Standard ) ) THEN
      ...
      CALL Compute_Std_Predictors( Pressure,      &  ! Input
                                   Temperature,   &  ! Input
                                   Water_Vapor,   &  ! Input
                                   Predictor( 1:MAX_N_STANDARD_PREDICTORS, : ) )  ! Output
    ...
    j_Absorber_loop: DO j = 1, SIZE( Absorber, DIM = 2 )
      ...
      CALL Compute_Int_Predictors( Pressure,             &  ! Input
                                   Temperature,          &  ! Input
                                   Absorber( 0:, j ),    &  ! Input
                                   Predictor( i1:i2, : ) )  ! Output
...
!
! PURPOSE:
!       Subroutine to calculate the predictors for the tangent-linear
...
!--------------------------------------------------------------------------------
  ...
  SUBROUTINE Compute_Predictors_TL ( Pressure,       &  ! Input,  K
    ...
    IF ( .NOT. PRESENT( No_Standard ) ) THEN
      ...
      CALL Compute_Std_Predictors_TL( Pressure,         &  ! Input
                                      Temperature,      &  ! Input
                                      Water_Vapor,      &  ! Input
                                      Pressure_TL,      &  ! Input
                                      Temperature_TL,   &  ! Input
                                      Water_Vapor_TL,   &  ! Input
                                      Predictor_TL( 1:MAX_N_STANDARD_PREDICTORS, : ) )  ! Output
    ...
    j_Absorber_Loop: DO j = 1, SIZE( Absorber_TL, DIM = 2 )
      ...
      CALL Compute_Int_Predictors_TL( Pressure,                &  ! Input
                                      Temperature,             &  ! Input
                                      Absorber( 0:, j ),       &  ! Input
                                      Pressure_TL,             &  ! Input
                                      Temperature_TL,          &  ! Input
                                      Absorber_TL( 0:, j ),    &  ! Input
                                      Predictor_TL( i1:i2, : ) )  ! Output
...
!
! PURPOSE:
!       Subroutine to calculate the predictors for the adjoint
!       gas absorption model.
!
! CATEGORY:
!       pCRTM
...
!
! CALLING SEQUENCE:
!       CALL Compute_Predictors_AD ( Pressure,       &  ! Input,   K
!                                    Temperature,    &  ! Input,   K
!                                    Water_Vapor,    &  ! Input,   K
!                                    Absorber,       &  ! Input,   0:K x J
!                                    Predictor_AD,   &  ! Input,   I x K
!                                    Pressure_AD,    &  ! Output,  K
!                                    Temperature_AD, &  ! Output,  K
!                                    Water_Vapor_AD, &  ! Output,  K
...
!                         amount array.
!                         UNITS:      Varies with absorber and predictor type.
!                         TYPE:       REAL( fp_kind )
...
!--------------------------------------------------------------------------------
  ...
  SUBROUTINE Compute_Predictors_AD ( Pressure,       &  ! Input, K
                                     Temperature,    &  ! Input, K
                                     Water_Vapor,    &  ! Input, K
                                     Absorber,       &  ! Input, 0:K x J
                                     Predictor_AD,   &  ! Input, I x K
    ...
    j_Absorber_Loop: DO j = 1, SIZE( Absorber, DIM = 2 )
      ...
      CALL Compute_Int_Predictors_AD( Pressure,                 &  ! Input,  K
                                      Temperature,              &  ! Input,  K
                                      Absorber( 0:, j ),        &  ! Input,  0:K
                                      Predictor_AD( i1:i2, : ), &  ! Input,  Iint x K
                                      Pressure_AD,              &  ! Output,  K
                                      Temperature_AD,           &  ! Output,  K
                                      Absorber_AD( 0:, j )      )  ! Output,  0:K
    ...
    IF ( .NOT. PRESENT( No_Standard ) ) THEN
      ...
      CALL Compute_Std_Predictors_AD( Pressure,                                       &  ! Input,  K
                                      Temperature,                                    &  ! Input,  K
                                      Water_Vapor,                                    &  ! Input,  K
...
!
! PURPOSE:
!       Subroutine to compute the standard, absorber independent predictors
...
!
! CALLING SEQUENCE:
!       CALL Compute_Std_Predictors( Pressure,    &  ! Input,  K
!                                    Temperature, &  ! Input,  K
!                                    Water_Vapor, &  ! Input,  K
...
!
! PURPOSE:
!       Subroutine to compute the integrated, absorber dependent predictors
!       for the gas absorption model.
!
! CATEGORY:
!       pCRTM
...
!
! CALLING SEQUENCE:
!       CALL Compute_Int_Predictors( Pressure,    &  ! Input,  K
!                                    Temperature, &  ! Input,  K
!                                    Absorber,    &  ! Input,  0:K
...
!--------------------------------------------------------------------------------
  ...
  SUBROUTINE Compute_Int_Predictors( Pressure,    &  ! Input,  K
    ...
    k_Layer_loop: DO k = 1, SIZE( Pressure )
      ...
      IF ( Absorber( k ) > MINIMUM_ABSORBER_AMOUNT ) THEN
...
!
! PURPOSE:
!       Subroutine to compute the standard, absorber independent
!       predictors for the tangent-linear gas absorption model.
!
! CATEGORY:
!       pCRTM
!
...
!--------------------------------------------------------------------------------
  ...
  SUBROUTINE Compute_Std_Predictors_AD( p,            &  ! Input,  K
                                        t,            &  ! Input,  K
                                        w,            &  ! Input,  K
                                        Predictor_AD, &  ! Input,  Istd x K
                                        p_AD,         &  ! Output, K
    ...
    ! -- Adjoint output
    REAL( fp_kind ), DIMENSION( : ),     INTENT( IN OUT ) :: p_AD          ! Output,  K
    REAL( fp_kind ), DIMENSION( : ),     INTENT( IN OUT ) :: t_AD          ! Output,  K
    REAL( fp_kind ), DIMENSION( : ),     INTENT( IN OUT ) :: w_AD          ! Output,  K
...
!--------------------------------------------------------------------------------
  ...
  SUBROUTINE Compute_Int_Predictors_AD( Pressure,       &  ! Input,  K
                                        Temperature,    &  ! Input,  K
                                        Absorber,       &  ! Input,  0:K
                                        Predictor_AD,   &  ! Input,  Iint x K
                                        Pressure_AD,    &  ! Output, K
...
!-------------------------------------------------------------------------------
!
! $Id: predictors.f90,v 3.8 2006/05/02 14:58:35 dgroff Exp $
!
...
! Revision 3.8  2006/05/02 14:58:35  dgroff
! - Replaced all references of Error_Handler with Message_Handler
!
! Revision 3.7  2004/12/22 17:43:33  paulv
! - Updated header documentation.
!
! Revision 3.6  2004/10/27 17:29:34  paulv
! - Added check for very low/zero absorber amounts in the integrated predictor
!   calculation routines. Previously, the following was done,
!     Inverse_1 = ONE / Absorber( k )
!     Inverse_2 = Inverse_1 * Inverse_1
!     Inverse_3 = Inverse_2 * Inverse_1
!     Inverse_4 = Inverse_2 * Inverse_2
!   which caused problems when the absorber amount was zero. This was changed
!   to
!     IF ( Absorber( k ) > MINIMUM_ABSORBER_AMOUNT ) THEN
!       Inverse_1 = ONE / Absorber( k )
!     ELSE
...
!     Inverse_3 = Inverse_2 * Inverse_1
!     Inverse_4 = Inverse_2 * Inverse_2
!   where the MINIMUM_ABSORBER_AMOUNT, defined in the PARAMETERS module, is
!   set to a value large enough such that Inverse_4 is still a valid number.
!
! Revision 3.5  2004/10/04 21:29:34  paulv
! - Removed inverse checks from all predictor calculations.
!
! Revision 3.4  2004/03/02 21:03:19  paulv
! - Cosmetic change only. Altered order of modules in USE statements.
!
