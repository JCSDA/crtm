...
!--------------------------------------------------------------------------------
  ...
  FUNCTION CRTM_K_Matrix_scalar( Atmosphere,   &  ! Input, Scalar
    ...
    INTEGER,      PARAMETER :: FW=1, KM=2  ! AllocStatus indices
    ! Local variables
    CHARACTER(256) :: Message
    ...
    AllocStatus(FW,1)=CRTM_Allocate_Predictor( Atmosphere%n_Layers   , &  ! Input
                                               ...
                                               Predictor             , &  ! Output
                                               Message_Log=Message_Log )  ! Error messaging
    AllocStatus(KM,1)=CRTM_Allocate_Predictor( Atmosphere%n_Layers   , &  ! Input
                                               MAX_N_PREDICTORS      , &  ! Input
                                               MAX_N_ABSORBERS       , &  ! Input
                                               Predictor_K           , &  ! Output
                                               Message_Log=Message_Log )  ! Error messaging
    AllocStatus(FW,2)=CRTM_Allocate_AtmAbsorption( Atmosphere%n_Layers   , &  ! Input
                                                   ...
                                                   Message_Log=Message_Log )  ! Error messaging
    AllocStatus(KM,2)=CRTM_Allocate_AtmAbsorption( Atmosphere%n_Layers   , &  ! Input
    ...
    AllocStatus(FW,3)=CRTM_Allocate_AtmScatter( Atmosphere%n_Layers,    &  ! Input
                                                ...
                                                CloudScatter,           &  ! Output
                                                Message_Log=Message_Log )  ! Error messaging
    AllocStatus(KM,3)=CRTM_Allocate_AtmScatter( Atmosphere%n_Layers,    &  ! Input
                                                MAX_N_LEGENDRE_TERMS,   &  ! Input
                                                MAX_N_PHASE_ELEMENTS,   &  ! Input
    ...
    AllocStatus(FW,4)=CRTM_Allocate_AtmScatter( Atmosphere%n_Layers,    &  ! Input
                                                ...
                                                Message_Log=Message_Log )  ! Error messaging
    AllocStatus(KM,4)=CRTM_Allocate_AtmScatter( Atmosphere%n_Layers,    &  ! Input
                                                MAX_N_LEGENDRE_TERMS,   &  ! Input
                                                MAX_N_PHASE_ELEMENTS,   &  ! Input
    ...
    AllocStatus(FW,5)=CRTM_Allocate_AtmScatter( Atmosphere%n_Layers,    &  ! Input
                                                ...
                                                AtmOptics,              &  ! Output
                                                Message_Log=Message_Log )  ! Error messaging
    AllocStatus(KM,5)=CRTM_Allocate_AtmScatter( Atmosphere%n_Layers,    &  ! Input
                                                MAX_N_LEGENDRE_TERMS,   &  ! Input
                                                MAX_N_PHASE_ELEMENTS,   &  ! Input
                                                AtmOptics_K,            &  ! Output
                                                Message_Log=Message_Log )  ! Error messaging
    ...
    AllocStatus(FW,6)=CRTM_Allocate_SfcOptics( MAX_N_ANGLES,           &  ! Input
                                               ...
                                               SfcOptics,              &  ! Output
                                               Message_Log=Message_Log )  ! Error messaging
    AllocStatus(KM,6)=CRTM_Allocate_SfcOptics( MAX_N_ANGLES,           &  ! Input
                                               MAX_N_STOKES,           &  ! Input
                                               SfcOptics_K,            &  ! Output
                                               Message_Log=Message_Log )  ! Error messaging
...
!          CYCLE Channel_Loop
    ...
    ! ---------------------------
    AllocStatus(KM,6)=CRTM_Destroy_SfcOptics( SfcOptics_K )
    AllocStatus(FW,6)=CRTM_Destroy_SfcOptics( SfcOptics )
    AllocStatus(KM,5)=CRTM_Destroy_AtmScatter( AtmOptics_K )
    AllocStatus(FW,5)=CRTM_Destroy_AtmScatter( AtmOptics )
    AllocStatus(KM,4)=CRTM_Destroy_AtmScatter( AerosolScatter_K )
    AllocStatus(FW,4)=CRTM_Destroy_AtmScatter( AerosolScatter )
    AllocStatus(KM,3)=CRTM_Destroy_AtmScatter( CloudScatter_K )
    AllocStatus(FW,3)=CRTM_Destroy_AtmScatter( CloudScatter )
    AllocStatus(KM,2)=CRTM_Destroy_AtmAbsorption( AtmAbsorption_K )
    AllocStatus(FW,2)=CRTM_Destroy_AtmAbsorption( AtmAbsorption )
    AllocStatus(KM,1)=CRTM_Destroy_Predictor( Predictor_K )
