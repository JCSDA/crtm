...
!!$#endif
  ...
  ! Profile dimensions
  INTEGER, PARAMETER :: N_PROFILES  = 4000
  ...
  !  CHARACTER(*), PARAMETER :: SENSOR_ID(N_SENSORS) = (/'cris-fsr_npp','amsua_n19   '/)
  CHARACTER(*), PARAMETER :: SENSOR_ID(N_SENSORS) = (/'atms_npp'/)!,'cris-fsr_npp',  &
!!$       'cris-fsr_npp','cris-fsr_npp','cris-fsr_npp','cris-fsr_npp','cris-fsr_npp','cris-fsr_npp', &
!!$       'cris-fsr_npp','cris-fsr_npp'/)
  ...
  Sensor_Loop: DO n = 1, N_SENSORS
    ...
    CALL Load_Atm_Data(rx)
    CALL Load_Sfc_Data(rx)
    DO m = 1, 2
    ...
    atm(3:N_PROFILES) = atm(2)
    sfc(3:N_PROFILES) = sfc(2)
    print *, 'h1'
    DO m = 3, N_PROFILES
       call random_number(rx)
       DO nc = 1, atm(m)%n_Clouds
          WHERE(atm(m)%Cloud(nc)%Water_Content > ZERO) atm(m)%Cloud_Fraction = rx
       END DO
    END DO
    print *, 'h2'
    ! 6b. Geometry input
...
CONTAINS
  ...
  !
  SUBROUTINE Load_Atm_Data(rx)
    ! Local variables
    INTEGER  :: nc
    INTEGER  :: k1, k2
    real(fp), intent(in) :: rx
    real(fp) :: rx2

    rx2 = 2.0_fp*rx - 1.0_fp
    ...
    !                                              (-1 : +1)*0.1 = -0.1*p : 0.1*p + p 
    atm(1)%Level_Pressure = atm(1)%Level_Pressure*rx2*0.1_fp + atm(1)%Level_Pressure
    ...
    atm(1)%Pressure = atm(1)%Pressure*rx2*0.1_fp + atm(1)%Pressure
    ...
    atm(1)%Temperature = atm(1)%Temperature*(2.0*rx - 1.0_fp)*0.1_fp + atm(1)%Temperature
    ...
    atm(1)%Absorber(:,1) = atm(1)%Absorber(:,1)*(2.0*rx - 1.0_fp)*0.1_fp + atm(1)%Absorber(:,1)
    ...
    atm(1)%Absorber(:,2) = atm(1)%Absorber(:,2)*(2.0*rx - 1.0_fp)*0.1_fp + atm(1)%Absorber(:,2)
    ...
    IF ( atm(1)%n_Absorbers > 2 ) THEN
      ...
      atm(1)%Absorber(:,3)     = 380.0_fp+380*(2.0_fp*rx-1.0_fp)*0.1_fp
    END IF
    ...
    IF ( atm(1)%n_Clouds > 0 ) THEN
      ...
      DO nc = 1, atm(1)%n_Clouds
        atm(1)%Cloud(nc)%Type = RAIN_CLOUD
        atm(1)%Cloud(nc)%Effective_Radius(k1:k2) = 100.0_fp + 100.0_fp*(2.0_fp*rx-1.0_fp) ! microns
        atm(1)%Cloud(nc)%Water_Content(k1:k2)    = 5.0_fp + 5.0_fp*(2.0_fp*rx-1.0_fp) ! kg/m^2
      END DO
    END IF
...
!!$    integer(i_kind), parameter :: SCRUB_SOIL = 19
    ...
    sfc(2)%Land_Type        = floor(rx*19)+1 !** 1 to 20
    if (sfc(2)%Land_Type == 10)  then
       sfc(2)%Land_Temperature = 273.15-rx*40 !** 233.15 to 273.15
    else
       sfc(2)%Land_Temperature = 273.15+rx*40 !** 273.15 to 313.15 
    end if
    sfc(2)%Lai              = rx 
    sfc(2)%Soil_Type        = floor(rx*7)+1 !** 1 to 8
    sfc(2)%Vegetation_Type  = floor(rx*11)+1 !** 1 to 12
