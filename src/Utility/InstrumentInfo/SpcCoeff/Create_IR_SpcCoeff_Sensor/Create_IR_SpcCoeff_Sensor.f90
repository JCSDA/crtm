...
PROGRAM Create_IR_SpcCoeff_Sensor
  ...
  '$Id: Create_IR_SpcCoeff_Sensor.f90,v 6.5 2006/08/15 20:28:54 wd20pd Exp $'
  ! Keyword set flag
...
!  REAL(fp), PARAMETER :: SOLAR_CUTOFF_WAVENUMBER = 775.0_fp
  ...
  REAL(fp), DIMENSION(N_TEMPERATURES) :: x_Temperature
  REAL(fp), DIMENSION(N_TEMPERATURES) :: y_Effective_Temperature
  REAL(fp), DIMENSION(N_TEMPERATURES) :: yFit_Effective_Temperature
  REAL(fp) :: var_Effective_Temperature
  REAL(fp), DIMENSION(:), ALLOCATABLE, TARGET :: Spectrum
  REAL(fp), DIMENSION(:), POINTER             :: Radiance   => NULL()
  REAL(fp), DIMENSION(:), POINTER             :: Irradiance => NULL()
  ...
  REAL(fp), DIMENSION(:), ALLOCATABLE :: Solar_Derivative

  ! Output header
  CALL Program_Message(PROGRAM_NAME, &
                       'Program to create the SENSOR infrared spectral '//&
                       'coefficient (SpcCoeff) data files from the '//&
                       'sensor SRF data files.', &
                       '$Revision: 6.5 $' )

  ! ---------------
  ! Get user inputs
  ! ---------------
  ! Read the SensorInfo filen
  ...
  n_Sensor_loop: DO n = 1, n_Sensors
    ...
    Infrared_Sensors: IF ( SensorInfo%Microwave_Flag == SENSORINFO_INFRARED       .AND. &
      ...
      ! SRF directly, so allocate the SpcCoeff structure
      ...
      Channel_Loop: DO l = 1, SpcCoeff%n_Channels
        ...
        ! Initialize the solar irradiance elements
  ...
  ! Destroy the solar data array/structures
...
!                        status. The error status codes are defined in the
...
!--------------------------------------------------------------------------------
  ...
  FUNCTION Least_Squares_Linear_Fit( x, y,         &  ! Input
    ...
    REAL(fp),     OPTIONAL, DIMENSION(:), INTENT(OUT) :: yFit
    ...
    REAL(fp) :: yAverage
    REAL(fp) :: sum_dx2
    REAL(fp), DIMENSION(SIZE(y)) :: yCalculated
