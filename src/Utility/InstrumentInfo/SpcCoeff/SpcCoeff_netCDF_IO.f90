...
MODULE SpcCoeff_netCDF_IO
  ...
  REAL(Double),  PARAMETER :: FP_FILLVALUE = -1.0_Double
  ...
  REAL(Double),  PARAMETER :: FREQUENCY_FILLVALUE        = FP_FILLVALUE
  REAL(Double),  PARAMETER :: WAVENUMBER_FILLVALUE       = FP_FILLVALUE
  REAL(Double),  PARAMETER :: PLANCK_C1_FILLVALUE        = FP_FILLVALUE
  REAL(Double),  PARAMETER :: PLANCK_C2_FILLVALUE        = FP_FILLVALUE
  REAL(Double),  PARAMETER :: BAND_C1_FILLVALUE          = FP_FILLVALUE
  REAL(Double),  PARAMETER :: BAND_C2_FILLVALUE          = FP_FILLVALUE
  REAL(Double),  PARAMETER :: CBR_FILLVALUE              = FP_FILLVALUE
  REAL(Double),  PARAMETER :: SOLAR_IRRADIANCE_FILLVALUE = FP_FILLVALUE
  ...
  INTEGER, PRIVATE, PARAMETER :: CHANNEL_FLAG_TYPE     = NF90_INT
...
!                                           AC_Release      =AC_Release      , &  ! Optional output
!                                           AC_Version      =AC_Version      , &  ! Optional output
...
!
! CALLING SEQUENCE:
!       Error_Status = Create_SpcCoeff_netCDF( NC_Filename                      , &  ! Input
!                                              n_Channels                       , &  ! Input
!                                              NC_FileID                        , &  ! Output
...
!------------------------------------------------------------------------------
  ...
  FUNCTION Create_SpcCoeff_netCDF( NC_Filename     , &  ! Input
    ...
    ! Check FOV input and set antenna correction flag
    AC_Present = .FALSE.
    ...
    ! ---------------------------
    Error_Status = Write_SpcCoeff_GAtts( NC_Filename                      , &
                                         NC_FileID                        , &
                                         Release         =Release         , &
                                         Version         =Version         , &
                                         Sensor_Id       =Sensor_Id       , &
    ...
    NF90_Status = NF90_DEF_VAR( NC_FileID, &
                                ...
                                varID =VarID )
    IF ( NF90_Status /= NF90_NOERR ) THEN
      Message = 'Error defining '//POLARIZATION_VARNAME//' variable in '//&
                TRIM(NC_Filename)//' - '//TRIM(NF90_STRERROR( NF90_Status ))
      GOTO 1000
    END IF
    ...
    IF ( ANY(Put_Status /= SUCCESS) ) THEN
      Message = 'Error writing '//POLARIZATION_VARNAME//&
                ' variable attributes to '//TRIM(NC_Filename)
      GOTO 1000
    END IF
    ...
    NF90_Status = NF90_DEF_VAR( NC_FileID, &
                                ...
                                varID =VarID )
    IF ( NF90_Status /= NF90_NOERR ) THEN
      Message = 'Error defining '//CHANNEL_FLAG_VARNAME//' variable in '//&
                TRIM(NC_Filename)//' - '//TRIM(NF90_STRERROR( NF90_Status ))
      GOTO 1000
    END IF
    ...
    Put_Status(1) = NF90_PUT_ATT( NC_FileID, &
                                  VarID, &
                                  LONGNAME_ATTNAME, &
                                  CHANNEL_FLAG_LONGNAME )
    Put_Status(2) = NF90_PUT_ATT( NC_FileID, &
                                  VarID, &
                                  DESCRIPTION_ATTNAME, &
                                  CHANNEL_FLAG_DESCRIPTION )
    Put_Status(3) = NF90_PUT_ATT( NC_FileID, &
                                  VarID, &
                                  UNITS_ATTNAME, &
                                  CHANNEL_FLAG_UNITS )
    Put_Status(4) = NF90_PUT_ATT( NC_FileID, &
                                  VarID, &
                                  FILLVALUE_ATTNAME, &
                                  CHANNEL_FLAG_FILLVALUE )
    IF ( ANY(Put_Status /= SUCCESS) ) THEN
      Message = 'Error writing '//CHANNEL_FLAG_VARNAME//&
                ' variable attributes to '//TRIM(NC_Filename)
      GOTO 1000
    END IF
    ...
    NF90_Status = NF90_DEF_VAR( NC_FileID, &
                                ...
                                varID =VarID )
    IF ( NF90_Status /= NF90_NOERR ) THEN
      Message = 'Error defining '//WAVENUMBER_VARNAME//' variable in '//&
                TRIM(NC_Filename)//' - '//TRIM(NF90_STRERROR( NF90_Status ))
      GOTO 1000
    END IF
    ...
    IF ( ANY(Put_Status /= SUCCESS) ) THEN
      Message = 'Error writing '//WAVENUMBER_VARNAME//&
                ' variable attributes to '//TRIM(NC_Filename)
      GOTO 1000
    END IF
    ...
    NF90_Status = NF90_DEF_VAR( NC_FileID, &
                                ...
                                varID =VarID )
    IF ( NF90_Status /= NF90_NOERR ) THEN
      Message = 'Error defining '//PLANCK_C1_VARNAME//' variable in '//&
                TRIM(NC_Filename)//' - '//TRIM(NF90_STRERROR( NF90_Status ))
      GOTO 1000
    END IF
    ...
    Put_Status(4) = NF90_PUT_ATT( NC_FileID, &
                                  ...
                                  PLANCK_C1_FILLVALUE )
    IF ( ANY(Put_Status /= SUCCESS) ) THEN
      Message = 'Error writing '//PLANCK_C1_VARNAME//&
                ' variable attributes to '//TRIM(NC_Filename)
      GOTO 1000
    END IF
    ...
    NF90_Status = NF90_DEF_VAR( NC_FileID, &
                                ...
                                varID =VarID )
    IF ( NF90_Status /= NF90_NOERR ) THEN
      Message = 'Error defining '//PLANCK_C2_VARNAME//' variable in '//&
                TRIM(NC_Filename)//' - '//TRIM(NF90_STRERROR( NF90_Status ))
      GOTO 1000
    END IF
    ...
    Put_Status(4) = NF90_PUT_ATT( NC_FileID, &
                                  ...
                                  PLANCK_C2_FILLVALUE )
    IF ( ANY(Put_Status /= SUCCESS) ) THEN
      Message = 'Error writing '//PLANCK_C2_VARNAME//&
                ' variable attributes to '//TRIM(NC_Filename)
      GOTO 1000
    END IF

    ! The polychromatic correction offset
    NF90_Status = NF90_DEF_VAR( NC_FileID, &
                                BAND_C1_VARNAME, &
                                BAND_C1_TYPE, &
                                dimIDs=(/n_Channels_DimID/), &
                                varID =VarID )
    IF ( NF90_Status /= NF90_NOERR ) THEN
      Message = 'Error defining '//BAND_C1_VARNAME//' variable in '//&
                TRIM(NC_Filename)//' - '//TRIM(NF90_STRERROR( NF90_Status ))
      GOTO 1000
    END IF
    ...
    Put_Status(4) = NF90_PUT_ATT( NC_FileID, &
                                  ...
                                  BAND_C1_FILLVALUE )
    IF ( ANY(Put_Status /= SUCCESS) ) THEN
      Message = 'Error writing '//BAND_C1_VARNAME//&
                ' variable attributes to '//TRIM(NC_Filename)
      GOTO 1000
    END IF

    ! The polychromatic correction slope
    NF90_Status = NF90_DEF_VAR( NC_FileID, &
                                BAND_C2_VARNAME, &
                                BAND_C2_TYPE, &
                                dimIDs=(/n_Channels_DimID/), &
                                varID =VarID )
    IF ( NF90_Status /= NF90_NOERR ) THEN
      Message = 'Error defining '//BAND_C2_VARNAME//' variable in '//&
                TRIM(NC_Filename)//' - '//TRIM(NF90_STRERROR( NF90_Status ))
      GOTO 1000
    END IF
    ...
    Put_Status(4) = NF90_PUT_ATT( NC_FileID, &
                                  ...
                                  BAND_C2_FILLVALUE )
    IF ( ANY(Put_Status /= SUCCESS) ) THEN
      Message = 'Error writing '//BAND_C2_VARNAME//&
                ' variable attributes to '//TRIM(NC_Filename)
      GOTO 1000
    END IF
    ...
    NF90_Status = NF90_DEF_VAR( NC_FileID, &
                                ...
                                varID =VarID )
    IF ( NF90_Status /= NF90_NOERR ) THEN
      Message = 'Error defining '//CBR_VARNAME//' variable in '//&
                TRIM(NC_Filename)//' - '//TRIM(NF90_STRERROR( NF90_Status ))
      GOTO 1000
    END IF
    ...
    Put_Status(4) = NF90_PUT_ATT( NC_FileID, &
                                  ...
                                  CBR_FILLVALUE )
    IF ( ANY(Put_Status /= SUCCESS) ) THEN
      Message = 'Error writing '//CBR_VARNAME//&
                ' variable attributes to '//TRIM(NC_Filename)
      GOTO 1000
    END IF


    ! The TOA solar irradiance
    NF90_Status = NF90_DEF_VAR( NC_FileID, &
                                ...
                                varID =VarID )
    IF ( NF90_Status /= NF90_NOERR ) THEN
      Message = 'Error defining '//SOLAR_IRRADIANCE_VARNAME//' variable in '//&
                TRIM(NC_Filename)//' - '//TRIM(NF90_STRERROR( NF90_Status ))
      GOTO 1000
    END IF
    ...
    IF ( ANY(Put_Status /= SUCCESS) ) THEN
      Message = 'Error writing '//SOLAR_IRRADIANCE_VARNAME//&
                ' variable attributes to '//TRIM(NC_Filename)
      GOTO 1000
    END IF
    ...
    IF ( AC_Present ) THEN
      Error_Status = DefineVar_AntCorr_netCDF( NC_Filename            , &
                                               ...
                                               Message_Log=Message_Log  )
      IF ( Error_Status /= SUCCESS ) THEN
        Message = 'Error defining AC variables in '//TRIM(NC_Filename)
        GOTO 1000
      END IF
    END IF
...
!------------------------------------------------------------------------------
  ...
  FUNCTION Inquire_SpcCoeff_netCDF( NC_Filename     , &  ! Input
    ...
    Error_Status = Open_SpcCoeff_netCDF( TRIM(NC_Filename), &
                                         ...
                                         Mode = 'READ' )
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'Error opening netCDF SpcCoeff data file '//&
                TRIM(NC_Filename)
      GOTO 2000
    END IF
    ...
    IF ( n_Dims > 1 ) THEN
      Error_Status = Get_netCDF_Dimension( NC_FileID, &
                                           ...
                                           Message_Log=Message_Log )
      IF ( Error_Status /= SUCCESS ) THEN
        Message = 'Error obtaining '//FOV_DIMNAME//&
                  ' dimension from '//TRIM(NC_Filename)
        GOTO 1000
      END IF
    END IF
...
!------------------------------------------------------------------------------
  ...
  FUNCTION Write_SpcCoeff_netCDF( NC_Filename , &  ! Input
    ...
    ! Set the AC release version GAtts
    ! --------------------------------
    ...
    Error_Status = Create_SpcCoeff_netCDF( NC_Filename                               , &  ! Input
                                           ...
                                           Release         =SpcCoeff%Release         , &  ! Optional input
    ...
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'Error writing '//POLARIZATION_VARNAME//' to '//TRIM(NC_Filename)
      GOTO 1000
    END IF
    ...
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'Error writing '//CHANNEL_FLAG_VARNAME//' to '//TRIM(NC_Filename)
      GOTO 1000
    END IF
    ...
    Error_Status = Put_netCDF_Variable( NC_FileID,   &
                                        ...
                                        SpcCoeff%Wavenumber )
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'Error writing '//WAVENUMBER_VARNAME//' to '//TRIM(NC_Filename)
      GOTO 1000
    END IF
    ...
    Error_Status = Put_netCDF_Variable( NC_FileID,   &
                                        ...
                                        SpcCoeff%Planck_C1 )
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'Error writing '//PLANCK_C1_VARNAME//' to '//TRIM(NC_Filename)
      GOTO 1000
    END IF
    ...
    Error_Status = Put_netCDF_Variable( NC_FileID,   &
                                        ...
                                        SpcCoeff%Planck_C2 )
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'Error writing '//PLANCK_C2_VARNAME//' to '//TRIM(NC_Filename)
      GOTO 1000
    END IF
    ...
    Error_Status = Put_netCDF_Variable( NC_FileID,   &
                                        ...
                                        SpcCoeff%Band_C1 )
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'Error writing '//BAND_C1_VARNAME//' to '//TRIM(NC_Filename)
      GOTO 1000
    END IF
    ...
    Error_Status = Put_netCDF_Variable( NC_FileID,   &
                                        ...
                                        SpcCoeff%Band_C2 )
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'Error writing '//BAND_C2_VARNAME//' to '//TRIM(NC_Filename)
      GOTO 1000
    END IF
    ...
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'Error writing '//CBR_VARNAME//' to '//TRIM(NC_Filename)
      GOTO 1000
    END IF
    ...
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'Error writing '//SOLAR_IRRADIANCE_VARNAME//' to '//TRIM(NC_Filename)
      GOTO 1000
    END IF
    ...
    IF ( SpcCoeff%AC_Present ) THEN
      Error_Status = WriteVar_AntCorr_netCDF( NC_Filename            , &
                                              ...
                                              Message_Log=Message_Log  )
      IF ( Error_Status /= SUCCESS ) THEN
        Message = 'Error writing AC variables to '//TRIM(NC_Filename)
        GOTO 2000
      END IF
    END IF
...
!------------------------------------------------------------------------------
  ...
  FUNCTION Read_SpcCoeff_netCDF( NC_Filename, &  ! Input
    ...
    CHARACTER(LEN(SpcCoeff%Sensor_Id)) :: Sensor_Id
    ...
    Error_Status = Inquire_SpcCoeff_netCDF( NC_Filename                      , &  ! Input
                                            ...
                                            Message_Log     =Message_Log       )  ! Error messaging
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'Error obtaining SpcCoeff dimension from '//TRIM(NC_Filename)
      GOTO 3000
    END IF
    ...
    Error_Status = Allocate_SpcCoeff( n_Channels             , &  ! Input
                                      ...
                                      Message_Log=Message_Log  )  ! Error messaging
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'Error occurred allocating SpcCoeff structure.'
      GOTO 3000
    END IF
    
    ! Assign the attributes to the structure
    SpcCoeff%Release             = Release         
    ...
    SpcCoeff%AC%Release          = AC_Release      
    SpcCoeff%AC%Version          = AC_Version      
    SpcCoeff%AC%Sensor_Id        = Sensor_Id       
    SpcCoeff%AC%WMO_Satellite_Id = WMO_Satellite_Id
    SpcCoeff%AC%WMO_Sensor_Id    = WMO_Sensor_Id   

    ! Check the release
    Error_Status = CheckRelease_SpcCoeff( SpcCoeff, &
                                          Message_Log = Message_Log )
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'SpcCoeff Release check failed for '//TRIM(NC_Filename)
      GOTO 3000
    END IF
    ...
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'Error opening netCDF SpcCoeff data file '//TRIM(NC_Filename)
      GOTO 2000
    END IF
    ...
    Error_Status = Get_netCDF_Variable( NC_FileID,   &
                                        ...
                                        SpcCoeff%Polarization )
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'Error reading '//POLARIZATION_VARNAME//' from '//TRIM(NC_Filename)
      GOTO 1000
    END IF
    Error_Status = Get_netCDF_Variable( NC_FileID,   &
                                        CHANNEL_FLAG_VARNAME, &
                                        SpcCoeff%Channel_Flag )
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'Error reading '//CHANNEL_FLAG_VARNAME//' from '//TRIM(NC_Filename)
      GOTO 1000
    END IF
    Error_Status = Get_netCDF_Variable( NC_FileID,   &
                                        FREQUENCY_VARNAME, &
    ...
    Error_Status = Get_netCDF_Variable( NC_FileID,   &
                                        ...
                                        SpcCoeff%Wavenumber )
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'Error reading '//WAVENUMBER_VARNAME//' from '//TRIM(NC_Filename)
      GOTO 1000
    END IF
    Error_Status = Get_netCDF_Variable( NC_FileID,   &
                                        PLANCK_C1_VARNAME, &
                                        SpcCoeff%Planck_C1 )
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'Error reading '//PLANCK_C1_VARNAME//' from '//TRIM(NC_Filename)
      GOTO 1000
    END IF
    Error_Status = Get_netCDF_Variable( NC_FileID,   &
                                        PLANCK_C2_VARNAME, &
                                        SpcCoeff%Planck_C2 )
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'Error reading '//PLANCK_C2_VARNAME//' from '//TRIM(NC_Filename)
      GOTO 1000
    END IF
    Error_Status = Get_netCDF_Variable( NC_FileID,   &
                                        BAND_C1_VARNAME, &
                                        SpcCoeff%Band_C1 )
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'Error reading '//BAND_C1_VARNAME//' from '//TRIM(NC_Filename)
      GOTO 1000
    END IF
    Error_Status = Get_netCDF_Variable( NC_FileID,   &
                                        BAND_C2_VARNAME, &
                                        SpcCoeff%Band_C2 )
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'Error reading '//BAND_C2_VARNAME//' from '//TRIM(NC_Filename)
      GOTO 1000
    END IF
    Error_Status = Get_netCDF_Variable( NC_FileID,   &
                                        CBR_VARNAME, &
                                        SpcCoeff%Cosmic_Background_Radiance )
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'Error reading '//CBR_VARNAME//' from '//TRIM(NC_Filename)
      GOTO 1000
    END IF
    Error_Status = Get_netCDF_Variable( NC_FileID,   &
                                        SOLAR_IRRADIANCE_VARNAME, &
                                        SpcCoeff%Solar_Irradiance )
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'Error reading '//SOLAR_IRRADIANCE_VARNAME//' from '//TRIM(NC_Filename)
      GOTO 1000
    END IF
    ...
    IF ( Destroy_Status /= SUCCESS ) &
      Message = TRIM(Message)//'; Error destroying SpcCoeff during error cleanup.'
