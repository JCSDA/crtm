...
!                                     DIMENSION:  Scalar
!
!         Max_Channels_per_Node:      The maximum number of sensor channels that
...
!                                     ATTRIBUTES: POINTER
!
!         MW_and_IR_Channel_Index:    The index array containing the combined microwave
!                                     and infrared channel indices. Each is mapped separately
!                                     (i.e. 1 to n_MW/IR_Channels) as the algorithms that
...
!                                     ATTRIBUTES: POINTER
!
!         Channel_Node_Map:           The index array to map nodes to channels.
!                                     UNITS:      N/A
!                                     TYPE:       INTEGER
...
!                                     ATTRIBUTES: POINTER
!
!         Node_Frequency:             Node frequencies in gigahertz.
!                                     UNITS:      Gigahertz (GHz)
!                                     TYPE:       REAL( Double )
!                                     DIMENSION:  Rank-1 (n_Nodes)
!                                     ATTRIBUTES: POINTER
!
!         Node_Wavenumber:            Node frequencies in inverse
!                                     centimetres.
!                                     UNITS:      inverse centimetres (cm^-1)
!                                     TYPE:       REAL( Double )
!                                     DIMENSION:  Rank-1 (n_Nodes)
!                                     ATTRIBUTES: POINTER
!
!         Node_Planck_C1:             First Planck function value for each
!                                     node.
!                                     UNITS:      mW/(m^2.sr.cm^-1)
!                                     TYPE:       REAL( Double )
!                                     DIMENSION:  Rank-1 (n_Nodes)
!                                     ATTRIBUTES: POINTER
!
!         Node_Planck_C2:             Second Planck function value for each
!                                     node.
!                                     UNITS:      K
!                                     TYPE:       REAL( Double )
!                                     DIMENSION:  Rank-1 (n_Nodes)
...
MODULE SpcCoeff_Define
  ...
  '$Id: SpcCoeff_Define.f90,v 6.8 2006/06/05 14:41:11 wd20pd Exp $'
  ...
  INTEGER, PRIVATE, PARAMETER :: SPCCOEFF_SPECTRAL_RELEASE = 2  ! This determines structure and file formats.
  ...
  TYPE, PUBLIC :: SpcCoeff_Spectral_type
    ...
    REAL( Double ),  POINTER, DIMENSION( : )    :: Solar_Irradiance           => NULL()  ! N
...
!--------------------------------------------------------------------------------
  ...
  FUNCTION Allocate_Spectral( n_Channels,            &  ! Input
    ...
    ALLOCATE( SpcCoeff%Sensor_Descriptor( n_Channels ),          &
              ...
              SpcCoeff%Solar_Irradiance( n_Nodes ),              &
...
!--------------------------------------------------------------------------------
  ...
  FUNCTION Concatenate_Spectral( SpcCoeff1,     &  ! Input/Output
    ...
    INTEGER :: Max_Channels_per_Node, ln2
    ...
    SpcCoeff1%Solar_Irradiance(n1:n2)           = SpcCoeff_Tmp%Solar_Irradiance
    ...
    SpcCoeff1%Solar_Irradiance(n1:n2)           = SpcCoeff2%Solar_Irradiance
...
!--------------------------------------------------------------------------------
  ...
  FUNCTION Equal_Spectral( SpcCoeff_LHS, &  ! Input
    ...
    Node_Loop: DO n = 1, SpcCoeff_RHS%n_Nodes
      ...
      IF ( .NOT. ( Compare_Float( SpcCoeff_LHS%Solar_Irradiance(n), &
                                  SpcCoeff_RHS%Solar_Irradiance(n), &
                                  ULP = ULP                         ) ) ) THEN
        Error_Status = FAILURE
        Message = Construct_Message( 'Solar_Irradiance', n, &
                                     SpcCoeff_LHS%Solar_Irradiance(n), &
                                     SpcCoeff_RHS%Solar_Irradiance(n), &
                                     Node = SET )
        CALL Display_Message( ROUTINE_NAME, &
                              TRIM( Message ), &
...
! Revision 6.8  2006/06/05 14:41:11  wd20pd
! Removed unused variables to prevent warning messages output by some
! compilers.
!
! Revision 6.7  2006/05/02 16:58:02  dgroff
! *** empty log message ***
!
! Revision 6.6  2005/09/27 18:14:44  paulv
! - Added Check_Once test for release/version number difference.
!
! Revision 6.5  2005/08/18 20:58:13  paulv
! - Fixed bug in Spectral Allocate() function. The Solar_Irradiance and
!   Is_Solar_Channel arrays were mixed up.
!
! Revision 6.4  2005/08/18 15:59:35  paulv
! - Moved the TOA solar irradiance in the Spectral SpcCoeff definition from
!   a channel-based array to a node-based array.
! - Updated Spectral SpcCoeff release to R2.
!
