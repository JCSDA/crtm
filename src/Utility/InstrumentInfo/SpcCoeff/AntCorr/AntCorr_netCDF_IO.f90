...
MODULE AntCorr_netCDF_IO
  ...
  USE AntCorr_Define , ONLY: AntCorr_type, &
                             ...
                             Info_AntCorr
  USE netcdf
  USE netCDF_Utility ,  Open_AntCorr_netCDF =>  Open_netCDF, &
                        Close_AntCorr_netCDF => Close_netCDF
...
!
! CALLING SEQUENCE:
!       Error_Status = Write_AntCorr_GAtts( NC_Filename                       , &  ! Input
!                                           NC_FileID                         , &  ! Input
...
!------------------------------------------------------------------------------
  ...
  FUNCTION Write_AntCorr_GAtts( NC_Filename     , &  ! Input
                                NC_FileID       , &  ! Input
    ...
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'Write_AntCorr_GAtts'
  ...
  END FUNCTION Write_AntCorr_GAtts
...
!
! CALLING SEQUENCE:
!       Error_Status = Read_AntCorr_GAtts( NC_Filename                      , &  ! Input
!                                          NC_FileID                        , &  ! Input
...
!------------------------------------------------------------------------------
  ...
  FUNCTION Read_AntCorr_GAtts( NC_Filename     , &  ! Input
                               NC_FileID       , &  ! Input
    ...
    INTEGER :: Error_Status
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'Read_AntCorr_GAtts'
    ! Local variables
    CHARACTER(256)  :: GAttName
  ...
  END FUNCTION Read_AntCorr_GAtts


!------------------------------------------------------------------------------
!
! NAME:
!       Create_AntCorr_netCDF
!
! PURPOSE:
!       Function to create a netCDF AntCorr data file for writing.
!
! CALLING SEQUENCE:
!       Error_Status = Create_AntCorr_netCDF( NC_Filename                      , &  ! Input
!                                             n_FOVs                           , &  ! Input
!                                             n_Channels                       , &  ! Input
!                                             NC_FileID                        , &  ! Output
!                                             Release         =Release         , &  ! Optional input
...
!------------------------------------------------------------------------------
  ...
  FUNCTION Create_AntCorr_netCDF( NC_Filename     , &  ! Input
                                  n_FOVs          , &  ! Input
                                  n_Channels      , &  ! Input
                                  NC_FileID       , &  ! Output
                                  Release         , &  ! Optional input
    ...
    INTEGER :: Error_Status
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'Create_AntCorr_netCDF'
    ! Local variables
    ...
    ! ---------------------------
    Error_Status = Write_AntCorr_GAtts( NC_Filename                      , &
                                        NC_FileID                        , &
                                        Release         =Release         , &
                                        Version         =Version         , &
                                        Sensor_Id       =Sensor_Id       , &
  ...
  END FUNCTION Create_AntCorr_netCDF
...
!------------------------------------------------------------------------------
  ...
  FUNCTION WriteVar_AntCorr_netCDF( NC_Filename, &
                                    NC_FileID  , &
                                    AntCorr    , &
                                    RCS_Id     , &  ! Revision control
                                    Message_Log) &
  ...
  FUNCTION ReadVar_AntCorr_netCDF( NC_Filename, &
                                   NC_FileID  , &
                                   AntCorr    , &
                                   RCS_Id     , &  ! Revision control
                                   Message_Log) &
...
!------------------------------------------------------------------------------
  ...
  FUNCTION Inquire_AntCorr_netCDF( NC_Filename     , &  ! Input
    ...
    ! Open the file
    ! -------------
    Error_Status = Open_AntCorr_netCDF( TRIM(NC_Filename), &
                                        NC_FileID, &
                                        Mode = 'READ' )
    IF ( Error_Status /= SUCCESS ) THEN
      Message = 'Error opening netCDF AntCorr data file '//&
                TRIM(NC_Filename)
      GOTO 2000
    END IF
    ...
    ! -------------------------
    Error_Status = Read_AntCorr_GAtts( NC_Filename                       , &
                                       NC_FileID                         , &
                                       Release          =Release         , &
                                       Version          =Version         , &
                                       Sensor_Id        =Sensor_Id       , &
    ...
    ! Close the file
    ! --------------
    Close_Status = Close_AntCorr_netCDF( NC_FileID )
    IF ( Close_Status /= SUCCESS ) THEN
      Error_Status = WARNING
      CALL Display_Message( ROUTINE_NAME, &
                            'Error closing netCDF AntCorr data file '// &
                            TRIM(NC_Filename), &
                            Error_Status, &
                            Message_Log = Message_Log )
    END IF
...
!------------------------------------------------------------------------------
  ...
  FUNCTION Write_AntCorr_netCDF( NC_Filename , &  ! Input
    ...
    Error_Status = Create_AntCorr_netCDF( NC_Filename                               , &  ! Input
                                          ...
                                          NC_FileID                                 , &  ! Output
                                          Release          =AntCorr%Release         , &  ! Optional input
...
!------------------------------------------------------------------------------
  ...
  FUNCTION Read_AntCorr_netCDF( NC_Filename, &  ! Input
    ...
    IF ( Destroy_Status /= SUCCESS ) &
      Message = TRIM(Message)//'; Error destroying AntCorr during error cleanup.'
