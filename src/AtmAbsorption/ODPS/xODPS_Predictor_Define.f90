...
!  END INTERFACE OPERATOR(==)
  ...
  TYPE :: ODPS_Predictor_type
    ...
    ! Dimension variables
    INTEGER :: Max_n_ODPS_Layers = 0 ! K   - maximum number of layers
    INTEGER :: n_ODPS_Layers     = 0 ! K   - actual number of layers
...
!--------------------------------------------------------------------------------
  ...
  ELEMENTAL SUBROUTINE ODPS_Predictor_Create( &
    ...
    ALLOCATE( self%Secant_Zenith(n_ODPS_Layers), &
              self%Component_ID(n_Components), &
              self%n_CP(n_Components), &
              self%X(n_ODPS_Layers, n_Predictors, n_Components), &
              self%Ref_Level_LnPressure(0:n_ODPS_Layers), &
    ...
    self%Max_n_ODPS_Layers = n_ODPS_Layers
    ...
    IF ( use_optran ) THEN
      ALLOCATE( self%OX(n_ODPS_Layers, MAX_OPTRAN_PREDICTORS), &
                self%Ap(n_ODPS_Layers, MAX_OPTRAN_ORDER), &
                self%dA(n_ODPS_Layers), &
                STAT = alloc_stat )
      IF ( alloc_stat /= 0 ) RETURN
      ! Initialise dimensions
...
!--------------------------------------------------------------------------------
  ...
  SUBROUTINE ODPS_Predictor_Inspect(self)
    ...
    WRITE(*,'(5x,"Secant_Zenith         :")'); WRITE(*,'(5(1x,es13.6,:))') self%Secant_Zenith
    WRITE(*,'(5x,"Component_ID          :")'); WRITE(*,'(10(1x,i3,:))')    self%Component_ID
    WRITE(*,'(5x,"n_CP                  :")'); WRITE(*,'(10(1x,i3,:))')    self%n_CP
    WRITE(*,'(5x,"X                     :")'); WRITE(*,'(5(1x,es13.6,:))') self%X
    ! ...Pressure profiles for interpolations
    WRITE(*,'(5x,"Ref_Level_LnPressure  :")'); WRITE(*,'(5(1x,es13.6,:))') self%Ref_Level_LnPressure
    WRITE(*,'(5x,"User_Level_LnPressure :")'); WRITE(*,'(5(1x,es13.6,:))') self%User_Level_LnPressure
