...
! the Optical Depth Pressure Space (ODPS) algorithm
!
!
! CREATION HISTORY:
!       Written by:     Yong Han, JCSDA, NOAA/NESDIS 20-Jun-2008
!       Add TL and AD:  Tong Zhu, CIRA/CSU@NOAA/NESDIS 18-Nov-2008
!
MODULE ODPS_Predictor
  ...
  PUBLIC  :: Compute_Predictor_TL
  PUBLIC  :: Compute_Predictor_AD
  PUBLIC  :: Compute_Predictor_OPTRAN
  PUBLIC  :: Compute_Predictor_OPTRAN_TL
  PUBLIC  :: Compute_Predictor_OPTRAN_AD
  PUBLIC  :: ODPS_APVariables_type
  PUBLIC  :: Get_max_n_Predicotrs
  PUBLIC  :: Get_n_Components
  PUBLIC  :: Get_n_Absorbers
  PUBLIC  :: Get_Component_ID
  PUBLIC  :: Get_Absorber_ID
  PUBLIC  :: Get_Ozone_Component_ID
  PUBLIC  :: Get_SaveFWVFlag
  ...
  CHARACTER( * ), PRIVATE, PARAMETER :: MODULE_RCS_ID = &
    '$Id: ODPS_Predictor.f90, YHan, July 10, 2008 Exp $'
...
!
! CALLING SEQUENCE:
!       CALL Compute_Predictor( Group_ID,           &  ! Input
!                               Temperature,        &  ! Input
!                               Absorber,           &  ! Input   
...
!------------------------------------------------------------------------------
  ...
  SUBROUTINE Compute_Predictor( Group_ID,           &
                                Temperature,        &   
                                Absorber,           &     
                                Ref_Level_Pressure, &     
                                Ref_Temperature,    &     
    ...
    ! Local variables
    ! ---------------
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'Compute_Predictor'
    INTEGER    ::    n_layers
    ...
    SELECT CASE( Group_ID )
      CASE( GROUP_1, GROUP_2 )
         CALL Compute_Predictor_IR()
      CASE( GROUP_3 )
         CALL Compute_Predictor_MW()
    END SELECT
    
CONTAINS

    SUBROUTINE Compute_Predictor_IR()
...
!        Predictor%X(k, 13, COMP_OZO_IR)  = SECANG(k)*GAzp(k,ABS_H2O_IR)                         
    ...
    END SUBROUTINE Compute_Predictor_IR

    SUBROUTINE Compute_Predictor_MW()
    ...
    END SUBROUTINE Compute_Predictor_MW
...
!------------------------------------------------------------------------------
  ...
  SUBROUTINE Compute_Predictor_TL( Group_ID,           &
    ...
    ! Local variables
    ! ---------------
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'Compute_Predictor_TL'
    INTEGER    ::    n_layers
    INTEGER    ::    k ! n_Layers, n_Levels
    INTEGER    ::    j ! n_absorbers
    REAL( fp ) ::    Tzp_sum_TL                           
    ...
    SELECT CASE( Group_ID )
      CASE( GROUP_1, GROUP_2 )
         CALL Compute_Predictor_IR_TL()
      CASE( GROUP_3 )
         CALL Compute_Predictor_MW_TL()
    END SELECT                     
...
CONTAINS
    ...
    SUBROUTINE Compute_Predictor_IR_TL()
...
!        Predictor_TL%X(k, 13, COMP_OZO_IR)  = SECANG(k)*GAzp_TL(k,ABS_H2O_IR)                         
    ...
    END SUBROUTINE Compute_Predictor_IR_TL

    SUBROUTINE Compute_Predictor_MW_TL()
    ...
    END SUBROUTINE Compute_Predictor_MW_TL

  END SUBROUTINE  Compute_Predictor_TL
...
!------------------------------------------------------------------------------
  ...
  SUBROUTINE Compute_Predictor_AD( Group_ID,           &
    ...
    ! Local variables
    ! ---------------
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'Compute_Predictor_AD'
    INTEGER    ::    n_layers
    INTEGER    ::    k ! n_Layers, n_Levels
    INTEGER    ::    j ! n_absorbers
    REAL( fp ) ::    Tzp_sum_AD
    ...
    SELECT CASE( Group_ID )
      CASE( GROUP_1, GROUP_2 )
         CALL Compute_Predictor_IR_AD()
      CASE( GROUP_3 )
         CALL Compute_Predictor_MW_AD()
    END SELECT                     
...
CONTAINS
    ...
    SUBROUTINE Compute_Predictor_IR_AD()
...
!                  + Predictor_AD%X(k, 13, COMP_OZO_IR)*SECANG(k)
    ...
    END SUBROUTINE Compute_Predictor_IR_AD

    SUBROUTINE Compute_Predictor_MW_AD()
    ...
    END SUBROUTINE Compute_Predictor_MW_AD

  END SUBROUTINE  Compute_Predictor_AD
...
!
! PURPOSE:
!       Subroutine to compute Compact OPTRAN predictors for water vapor line
!       absorption.
!
! CALLING SEQUENCE:
!       CALL SUBROUTINE Compute_Predictor_OPTRAN(Temperature,    &  ! Input
!                                                vapor,          &  ! Inpt
...
!------------------------------------------------------------------------------
  ...
  SUBROUTINE Compute_Predictor_OPTRAN(Temperature,    &
                                      Vapor,          &
                                      Level_Pressure, &
                                      Pressure,       &
                                      secant_angle,   &
  ...
  END SUBROUTINE Compute_Predictor_OPTRAN
...
!
! PURPOSE:
!       Subroutine to compute TL Compact OPTRAN predictors for water vapor line
!       absorption.
!
! CALLING SEQUENCE:
...
!------------------------------------------------------------------------------
  ...
  END SUBROUTINE Compute_Predictor_OPTRAN_TL
...
!
! PURPOSE:
!       Subroutine to compute AD Compact OPTRAN predictors for water vapor line
!       absorption.
!
! CALLING SEQUENCE:
...
!------------------------------------------------------------------------------
  ...
  END SUBROUTINE Compute_Predictor_OPTRAN_AD

  FUNCTION Get_max_n_Predicotrs( Group_Index ) RESULT( max_n_Predictors ) 
  ...
  END FUNCTION Get_max_n_Predicotrs

  FUNCTION Get_n_Components( Group_Index ) RESULT( n_Components ) 
    INTEGER, INTENT( IN ) :: Group_Index
  ...
  FUNCTION Get_n_Absorbers( Group_Index ) RESULT( n_Absorbers ) 
    INTEGER, INTENT( IN ) :: Group_Index
  ...
  END FUNCTION Get_n_Absorbers

  FUNCTION Get_Component_ID(Component_Index, Group_Index) RESULT( Component_ID )
  ...
  END FUNCTION Get_Component_ID

  FUNCTION Get_Absorber_ID(Absorber_Index, Group_Index) RESULT(  Absorber_ID )
  ...
  END FUNCTION Get_Absorber_ID

  FUNCTION Get_Ozone_Component_ID(Group_Index) RESULT( Ozone_Component_ID )
    INTEGER, INTENT(IN) :: Group_Index
