...
MODULE CRTM_AerosolScatter
  ...
  TYPE, PUBLIC :: CRTM_ASVariables_type
    ...
    REAL(fp), DIMENSION(0:MAX_N_LEGENDRE_TERMS,&
                        ...
                        MAX_N_LAYERS,          &
                        MAX_N_AEROSOLS         )  :: pcoeff = ZERO    !p      ! Phase coefficients
...
!------------------------------------------------------------------------------
  ...
  FUNCTION CRTM_Compute_AerosolScatter_AD( Atmosphere,        &  ! Input
    ...
    REAL(fp)  :: p_coef_AD(0:AerosolScatter%n_Legendre_Terms, AerosolScatter%n_Phase_Elements)
    REAL(fp)  :: bs, bs_AD
    ...
    Aerosol_loop: DO n = 1, Atmosphere%n_Aerosols
      ...
      Aerosol_Layer_loop: DO k = 1, nAerosol_Layers
        ...
        IF( n_Phase_Elements > 0 ) THEN
          DO m = 1, n_Phase_Elements
            DO l = 0, n_Legendre_Terms
              ...
              p_coef_AD(l,m) = p_coef_AD(l,m) + (bs * AerosolScatter_AD%Phase_Coefficient(l,m,ka))
            END DO
          END DO
        END IF
        ...
        Atmosphere_AD%Aerosol(n)%Concentration(ka) = Atmosphere_AD%Aerosol(n)%Concentration(ka) + &
                                                     (ASV%ext(ka,n) * AerosolScatter_AD%Optical_Depth(ka))
        ext_AD = ext_AD + (Atmosphere%Aerosol(n)%Concentration(ka) * AerosolScatter_AD%Optical_Depth(ka))
        ...
        ext_AD = ext_AD + (Atmosphere%Aerosol(n)%Concentration(ka) * ASV%w0(ka,n) * bs_AD )
        w0_AD  = w0_AD  + (Atmosphere%Aerosol(n)%Concentration(ka) * ASV%ext(ka,n) * bs_AD )
        Atmosphere_AD%Aerosol(n)%Concentration(ka) = Atmosphere_AD%Aerosol(n)%Concentration(ka) + &
                                                     (ASV%w0(ka,n) * ASV%ext(ka,n) * bs_AD)
...
!################################################################################
  ...
  SUBROUTINE Get_Aerosol_Opt(AerosolScatter,  &  ! Input AerosolScatter structure
    ...
    CALL interp_2D(AeroC%ke(i1:i2,j1:j2,k), wlp, xlp, ext)
    CALL interp_2D(AeroC%w(i1:i2,j1:j2,k), wlp, xlp, w0)
    ...
    IF (AerosolScatter%n_Phase_Elements > 0 .AND.  &
      ...
      DO l = 0, AerosolScatter%n_Legendre_Terms
        CALL interp_2D(AeroC%pcoeff(i1:i2,j1:j2,k,l+AerosolScatter%lOffset), &
                       wlp, xlp, pcoeff(l,1))
      END DO
    END IF
  ...
  SUBROUTINE Get_Aerosol_Opt_TL(AerosolScatter_TL, &  ! Input  AerosolScatterTL structure
    ...
    ! Perform Interpolation
    CALL interp_2D_TL(AeroC%ke(i1:i2,j1:j2,k), wlp, xdlp, r_int_TL, ext_TL)
    CALL interp_2D_TL(AeroC%w(i1:i2,j1:j2,k), wlp, xdlp, r_int_TL, w0_TL)
    CALL interp_2D_TL(AeroC%g(i1:i2,j1:j2,k), wlp, xdlp, r_int_TL, g_TL)
    IF (AerosolScatter_TL%n_Phase_Elements > 0 .AND. &
      ...
      DO l = 0, AerosolScatter_TL%n_Legendre_Terms
        CALL interp_2D_TL(AeroC%pcoeff(i1:i2,j1:j2,k,l+AerosolScatter_TL%lOffset), &
  ...
  SUBROUTINE Get_Aerosol_Opt_AD(AerosolScatter_AD, & ! Input AerosolScatter AD structure
                                ...
                                ext_AD           , & ! AD Input extinction cross section
                                w0_AD            , & ! AD Input single scatter albedo
                                g_AD             , & ! AD Input asymmetry factor
                                p_coeff_AD       , & ! AD Input spherical Legendre coefficients
    ...
    REAL(fp),                   INTENT(IN OUT) :: p_coeff_AD(0:,:) ! AD Input
    ...
    ! Perform Interpolation
    CALL interp_2D_AD(AeroC%ke(i1:i2,j1:j2,k), wlp, xdlp, ext_AD, Reff_AD)
    CALL interp_2D_AD(AeroC%w(i1:i2,j1:j2,k), wlp, xdlp, w0_AD, Reff_AD)
    ...
    IF (AerosolScatter_AD%n_Phase_Elements > 0 .AND. &
      ...
      DO l = 0, AerosolScatter_AD%n_Legendre_Terms
        CALL interp_2D_AD(AeroC%pcoeff(i1:i2,j1:j2,k,l+AerosolScatter_AD%lOffset), &
                                            wlp, xdlp, p_coeff_AD(l,1), Reff_AD)
      END DO
    END IF
