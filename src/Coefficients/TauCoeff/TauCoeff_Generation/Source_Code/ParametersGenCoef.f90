...
module ParametersGenCoef
  ...
  real(fp_kind),parameter :: DTAU_THRESHOLD = 0.005  
  ...
  real(fp_kind),parameter :: VirtEmiss           = 0.98_fp_kind  ! virtial emissivity
...
!                                            4._fp_kind /)      ! criterion for the check the sensitivity (ozo)
  ...
  character(len=10)   :: SatName                ! Satellite name
  character(len=10)   :: SenName                ! Sensor name
  integer             :: SatIdWmo               ! Sat ID (WMO)
  integer             :: SenIdWmo               ! Sen ID (WMO)
  integer             :: SenIdNcep              ! Sen ID (NCEP)

  type(SpcCoeff_type) :: SpcCoeff               ! spectrum coef (freq, Plunck const)
...
!
!  --- Add character parameter OutFileName_TransCoeff = "TauCoeff.nc"
!  --- Add variables for history information
! 
!  --- move definations for IdMolecule_Dry,IdMolecule_Wet,IdMolecule_Ozo from
!      ReadProfile_netCDF.f90 to this module.
