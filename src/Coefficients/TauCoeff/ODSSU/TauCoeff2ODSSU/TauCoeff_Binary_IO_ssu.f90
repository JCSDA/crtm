...
MODULE TauCoeff_Binary_IO_ssu
  ...
  USE TauCoeff_Define_ssu,     ONLY: TauCoeff_ssu_Type, &
                                 N_TAUCOEFF_ITEMS_ssu, &
                                 TAUCOEFF_DATA_TYPE_ssu, &
                                 TAUCOEFF_DATA_NAME_ssu, &
  ...
  ! ------------
  PRIVATE
  PUBLIC :: Inquire_TauCoeff_Binary_ssu
  PUBLIC :: Read_TauCoeff_Binary_ssu
  PUBLIC :: Write_TauCoeff_Binary_ssu
...
!
! NAME:
!       Inquire_TauCoeff_Binary_ssu
!
! PURPOSE:
!       Function to inquire a Binary format TauCoeff file.
!
! CALLING SEQUENCE:
!       Error_Status = Inquire_TauCoeff_Binary_ssu( Filename,                    &  ! Input
...
!                                               n_TC_CellPressures = n_TC_CellPressures,   &  ! Optional output
!                                               n_Ref_CellPressures = n_Ref_CellPressures,   &  ! Optional output
...
!------------------------------------------------------------------------------
  ...
  FUNCTION Inquire_TauCoeff_Binary_ssu( Filename,     &  ! Input
                                    n_Orders,     &  ! Optional output
                                    n_Predictors, &  ! Optional output
                                    n_Absorbers,  &  ! Optional output
                                    n_Channels,   &  ! Optional output
                                    n_TC_CellPressures,   &  ! Optional output
                                    n_Ref_CellPressures,   &  ! Optional output
    ...
    ! Function parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'Inquire_TauCoeff_Binary_ssu'
    ! Function variables
    ...
    INTEGER(Long) :: File_n_TC_CellPressures
    INTEGER(Long) :: File_n_Ref_CellPressures
    ...
    READ( FileID, IOSTAT = IO_Status ) File_n_Orders , &
                                       ...
                                       File_n_Absorbers, &
                                       File_n_Channels,  &
                                       File_n_TC_CellPressures, &
                                       File_n_Ref_CellPressures
    IF ( IO_Status /= 0 ) THEN
      Error_Status = FAILURE
    ...
    IF ( PRESENT( n_Channels   ) ) n_TC_CellPressures  = File_n_TC_CellPressures
    IF ( PRESENT( n_Channels   ) ) n_Ref_CellPressures  = File_n_Ref_CellPressures
    IF ( PRESENT( Release      ) ) Release      = File_Release
    IF ( PRESENT( Version      ) ) Version      = File_Version

  END FUNCTION Inquire_TauCoeff_Binary_ssu


!------------------------------------------------------------------------------
!
! NAME:
!       Read_TauCoeff_Binary_ssu
!
! PURPOSE:
!       Function to read Binary format TauCoeff files.
!
! CALLING SEQUENCE:
!       Error_Status = Read_TauCoeff_Binary_ssu( Filename,                              &  ! Input
!                                            TauCoeff,                              &  ! Output
...
!------------------------------------------------------------------------------
  ...
  FUNCTION Read_TauCoeff_Binary_ssu( Filename,          &  ! Input
                                 TauCoeff,          &  ! Output
    ...
    INTEGER :: Error_Status
    ! Function parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'Read_TauCoeff_Binary_ssu'
    ! Function variables
    ...
    INTEGER(Long), DIMENSION(N_TAUCOEFF_ITEMS_ssu) :: Data_Type
    ...
    ! Check the release
    Error_Status = Check_TauCoeff_Release_ssu( TauCoeff, &
                                           Message_Log = Message_Log )
    IF ( Error_Status /= SUCCESS ) THEN
    ...
    IF ( n_Items /= N_TAUCOEFF_ITEMS_ssu ) THEN
      WRITE( Message, '( "Number of data items in ", a, " (", i2, &
                        &") is inconsistent with definition (", i2, ")." )' ) &
                      TRIM( Filename ), n_Items, N_TAUCOEFF_ITEMS_ssu
      GOTO 2000
    END IF
    ...
    DO n = 1, n_Items
      IF ( Data_Type(n) /= TAUCOEFF_DATA_TYPE_ssu(n) ) THEN
        WRITE( Message, '( "Invalid type for data item #", i2, &
                          &", ", a, ", in ", a )' ) &
                        n, TRIM( TAUCOEFF_DATA_NAME_ssu(n) ), TRIM( Filename )
        GOTO 2000
      END IF
    END DO

    ! Allocate the TauCoeff structure for reading
    Error_Status = Allocate_TauCoeff_ssu( n_Orders, &
                                      n_Predictors, &
                                      n_Absorbers, &
                                      n_Channels, &
                                      n_TC_CellPressures, &
    ...
    ! Read the TC_CellPressure 
    READ( FileID, IOSTAT = IO_Status ) TauCoeff%TC_CellPressure
    IF ( IO_Status /= 0 ) THEN
      WRITE( Message, '( "Error reading TC_CellPressure from ", a, &
                        &". IOSTAT = ", i5 )' ) &
                      TRIM( Filename ), IO_Status
      GOTO 1000
    END IF
    ...
    ! Read the Ref_Time 
    READ( FileID, IOSTAT = IO_Status ) TauCoeff%Ref_Time
    IF ( IO_Status /= 0 ) THEN
      WRITE( Message, '( "Error reading Ref_Time from ", a, &
                        &". IOSTAT = ", i5 )' ) &
                      TRIM( Filename ), IO_Status
      GOTO 1000
    END IF

    ! Read the Ref_CellPressure 
    READ( FileID, IOSTAT = IO_Status ) TauCoeff%Ref_CellPressure
    IF ( IO_Status /= 0 ) THEN
      WRITE( Message, '( "Error reading Ref_CellPressure from ", a, &
                        &". IOSTAT = ", i5 )' ) &
                      TRIM( Filename ), IO_Status
      GOTO 1000
    END IF
    ...
    ! Count the sensors
    CALL Count_TauCoeff_Sensors_ssu(TauCoeff)

    ! Output an info message
    IF ( Noisy ) THEN
      CALL Info_TauCoeff_ssu( TauCoeff, Message )
      CALL Display_Message( ROUTINE_NAME, &
    ...
    1000 CONTINUE
    Destroy_Status = Destroy_TauCoeff_ssu(TauCoeff, Message_Log=Message_Log)
  ...
  END FUNCTION Read_TauCoeff_Binary_ssu


!------------------------------------------------------------------------------
!
! NAME:
!       Write_TauCoeff_Binary_ssu
!
! PURPOSE:
!       Function to write Binary format TauCoeff files.
!
! CALLING SEQUENCE:
!       Error_Status = Write_TauCoeff_Binary_ssu( Filename,                 &   ! Input
!                                             TauCoeff,                 &   ! Input
!                                             Quiet       = Quiet,      &   ! Optional input
!                                             RCS_Id      = RCS_Id,     &   ! Revision control
!                                             Message_Log = Message_Log )   ! Error messaging
...
!------------------------------------------------------------------------------
  ...
  FUNCTION Write_TauCoeff_Binary_ssu( Filename,     &  ! Input
                                  TauCoeff,     &  ! Input
                                  Quiet,        &  ! Optional input
                                  RCS_Id,       &  ! Revision control
                                  Message_Log ) &  ! Error messaging
    ...
    INTEGER :: Error_Status
    ! Function parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'Write_TauCoeff_Binary_ssu'
    ...
    ! Check structure
    IF ( .NOT. Associated_TauCoeff_ssu( TauCoeff ) ) THEN
    ...
    IF ( TauCoeff%n_Orders     < 1 .OR. &
         ...
         TauCoeff%n_TC_CellPressures  < 1 .OR. &
         TauCoeff%n_Ref_CellPressures  < 1 .OR. &
    ...
    WRITE( FileID, IOSTAT = IO_Status ) N_TAUCOEFF_ITEMS_ssu
    IF ( IO_Status /= 0 ) THEN
    ...
    WRITE( FileID, IOSTAT = IO_Status ) TAUCOEFF_DATA_TYPE_ssu
    IF ( IO_Status /= 0 ) THEN
    ...
    ! Write the TC_CellPressure 
    WRITE( FileID, IOSTAT = IO_Status ) TauCoeff%TC_CellPressure
    IF ( IO_Status /= 0 ) THEN
      WRITE( Message, '( "Error writing TC_CellPressure to ", a, &
                        &". IOSTAT = ", i5 )' ) &
                      TRIM( Filename ), IO_Status
      GOTO 1000
    END IF
    ...
    ! Write the Ref_Time 
    WRITE( FileID, IOSTAT = IO_Status ) TauCoeff%Ref_Time
    IF ( IO_Status /= 0 ) THEN
      WRITE( Message, '( "Error writing Ref_Time to ", a, &
                        &". IOSTAT = ", i5 )' ) &
                      TRIM( Filename ), IO_Status
      GOTO 1000
    END IF

    ! Write the Ref_CellPressure 
    WRITE( FileID, IOSTAT = IO_Status ) TauCoeff%Ref_CellPressure
    IF ( IO_Status /= 0 ) THEN
      WRITE( Message, '( "Error writing Ref_CellPressure to ", a, &
                        &". IOSTAT = ", i5 )' ) &
                      TRIM( Filename ), IO_Status
      GOTO 1000
    END IF
  ...
  END FUNCTION Write_TauCoeff_Binary_ssu

END MODULE TauCoeff_Binary_IO_ssu
