...
! *!IMPORTANT!*
! -------------
! Note that the TauCoeff_ssu_type is PUBLIC and its members are not
...
MODULE TauCoeff_Define_ssu
  ...
  PRIVATE
  ! Parameters
  PUBLIC :: N_TAUCOEFF_ITEMS_ssu
  PUBLIC :: TAUCOEFF_DATA_TYPE_ssu
  PUBLIC :: TAUCOEFF_DATA_NAME_ssu
  ! Datatypes
  PUBLIC :: TauCoeff_ssu_type
  ! Procedures
  PUBLIC :: Associated_TauCoeff_ssu
  PUBLIC :: Destroy_TauCoeff_ssu
  PUBLIC :: Allocate_TauCoeff_ssu
  PUBLIC :: Assign_TauCoeff_ssu
  PUBLIC :: sConcatenate_Channel_TauCoeff
  PUBLIC :: sConcatenate_Absorber_TauCoeff
  PUBLIC :: Equal_TauCoeff_ssu
  PUBLIC :: Check_TauCoeff_Release_ssu
  PUBLIC :: Count_TauCoeff_Sensors_ssu
  PUBLIC :: Info_TauCoeff_ssu


  ! -----------------
  ! Module parameters
  ! -----------------
  INTEGER(Long), PARAMETER :: TC_SSU_Algorithm_ID = 1
  ...
  INTEGER, PARAMETER :: TC_SSU_RELEASE = 5  ! This determines structure and file formats.
  INTEGER, PARAMETER :: TC_SSU_VERSION = 4  ! This is just the data version.
  ! Number of TauCoeff data items
  INTEGER(Long), PARAMETER :: N_TAUCOEFF_ITEMS_ssu = 12_Long
  ...
  INTEGER(Long), PARAMETER, DIMENSION( N_TAUCOEFF_ITEMS_ssu ) :: &
    TAUCOEFF_DATA_TYPE_ssu = (/ 7_Long, &  ! Sensor_Descriptor
  ...
  CHARACTER(*), PARAMETER, DIMENSION( N_TAUCOEFF_ITEMS_ssu ) :: &
    TAUCOEFF_DATA_NAME_ssu = (/ 'Sensor_Descriptor', &
                            'NCEP_Sensor_ID   ', &
                            'WMO_Satellite_ID ', &
                            'WMO_Sensor_ID    ', &
                            'Sensor_Channel   ', &
  ...
  ! Ref_Time and Ref_CellPressure. 
    ...
    INTEGER(Long) :: Release = TC_SSU_RELEASE
    INTEGER(Long) :: Version = TC_SSU_VERSION

    INTEGER(Long) :: Algorithm_ID = TC_SSU_Algorithm_ID
    ! Array dimensions
    ...
    INTEGER(Long), POINTER, DIMENSION(:,:,:,:) :: Order_Index => NULL()  ! 0:Iuse x J x L x M
    ...
    INTEGER(Long), POINTER, DIMENSION(:,:,:,:)    :: Predictor_Index => NULL()  ! 0:Iuse x J x L x M
    ! The array of coefficients
    REAL(Double),  POINTER, DIMENSION(:,:,:,:,:) :: C => NULL() ! 0:Iorder x 0:Iuse x J x L x M
  END TYPE TauCoeff_ssu_type
...
!
! CALLING SEQUENCE:
!       CALL Clear_TauCoeff_ssu( TauCoeff ) ! Output
!
! OUTPUT ARGUMENTS:
...
!----------------------------------------------------------------------------------
  ...
  SUBROUTINE Clear_TauCoeff_ssu( TauCoeff )
    TYPE(TauCoeff_ssu_type), INTENT(IN OUT) :: TauCoeff
    TauCoeff%n_Orders     = 0
    TauCoeff%n_Predictors = 0
    TauCoeff%n_Absorbers  = 0
    TauCoeff%n_Channels   = 0
    TauCoeff%n_TC_CellPressures   = 0
    TauCoeff%n_Ref_CellPressures   = 0
    TauCoeff%StrLen       = DL
    TauCoeff%n_Sensors = 0
  END SUBROUTINE Clear_TauCoeff_ssu
...
!
! NAME:
!       Associated_TauCoeff_ssu
!
! PURPOSE:
!       Function to test the association status of the pointer members of a
!       TauCoeff_ssu structure.
!
! CALLING SEQUENCE:
!       Association_Status = Associated_TauCoeff_ssu( TauCoeff,           &  ! Input
...
!--------------------------------------------------------------------------------
  ...
  FUNCTION Associated_TauCoeff_ssu( TauCoeff,  & ! Input
                                ANY_Test ) & ! Optional input
                              RESULT( Association_Status )
    ! Arguments
    TYPE(TauCoeff_ssu_type), INTENT(IN) :: TauCoeff
    ...
    IF ( ALL_Test ) THEN
      IF ( ASSOCIATED( TauCoeff%Sensor_Descriptor ) .AND. &
           ...
           ASSOCIATED( TauCoeff%C                 ) .AND. &    
           ASSOCIATED( TauCoeff%TC_CellPressure      ) .AND. &   
           ASSOCIATED( TauCoeff%Ref_Time) .AND. &  
           ASSOCIATED( TauCoeff%Ref_CellPressure     )       ) THEN
    ...
    ELSE
      IF ( ASSOCIATED( TauCoeff%Sensor_Descriptor ) .OR. &
           ...
           ASSOCIATED( TauCoeff%C                 ) .OR. & 
           ASSOCIATED( TauCoeff%TC_CellPressure      ) .OR. &   
           ASSOCIATED( TauCoeff%Ref_Time     ) .OR. &  
           ASSOCIATED( TauCoeff%Ref_CellPressure     )       ) THEN
        Association_Status = .TRUE.
      END IF
    END IF

  END FUNCTION Associated_TauCoeff_ssu


!------------------------------------------------------------------------------
!
! NAME:
!       Destroy_TauCoeff_ssu
! 
! PURPOSE:
...
!
! CALLING SEQUENCE:
!       Error_Status = Destroy_TauCoeff_ssu( TauCoeff,                 &  ! Output
...
!------------------------------------------------------------------------------
  ...
  FUNCTION Destroy_TauCoeff_ssu( TauCoeff,     &  ! Output
    ...
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'Destroy_TauCoeff_ssu'
    ! Local variables
    ...
    IF ( Clear ) CALL Clear_TauCoeff_ssu( TauCoeff )

    ! If ALL pointer members are NOT associated, do nothing
    IF ( .NOT. Associated_TauCoeff_ssu(TauCoeff) ) RETURN
    ...
    DEALLOCATE( TauCoeff%Sensor_Descriptor, &
                ...
                TauCoeff%TC_CellPressure     , &
                TauCoeff%Ref_Time    , &
                TauCoeff%Ref_CellPressure    , &
                TauCoeff%C                , &
  ...
  END FUNCTION Destroy_TauCoeff_ssu


!------------------------------------------------------------------------------
!
! NAME:
!       Allocate_TauCoeff_ssu
! 
! PURPOSE:
...
!
! CALLING SEQUENCE:
!       Error_Status = Allocate_TauCoeff_ssu( n_Orders,                  &  ! Input
!                                         n_Predictors,              &  ! Input
!                                         n_Absorbers,               &  ! Input
!                                         n_Channels,                &  ! Input
...
!------------------------------------------------------------------------------
  ...
  FUNCTION Allocate_TauCoeff_ssu( n_Orders,        &  ! Input
                              n_Predictors,    &  ! Input
                              n_Absorbers,     &  ! Input
                              n_Channels,      &  ! Input
                              n_TC_CellPressures,  &  ! Input
    ...
    INTEGER :: Error_Status
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'Allocate_TauCoeff_ssu'
    ! Local variables
    CHARACTER(256)  :: Message
    INTEGER :: Allocate_Status
    ...
    IF ( Associated_TauCoeff_ssu( TauCoeff, ANY_Test=1 ) ) THEN
    ...
    ALLOCATE( TauCoeff%Sensor_Descriptor( n_Channels ), &
              ...
              TauCoeff%Order_Index( 0:n_Predictors, n_Absorbers, n_Channels, n_TC_CellPressures ), &
              TauCoeff%Predictor_Index( 0:n_Predictors, n_Absorbers, n_Channels, n_TC_CellPressures ), &
              TauCoeff%TC_CellPressure( n_TC_CellPressures,n_Channels ), &
              TauCoeff%Ref_Time(n_Ref_CellPressures ), &
              TauCoeff%Ref_CellPressure(n_Ref_CellPressures,n_Channels ), &
              TauCoeff%C( 0:n_Orders, 0:n_Predictors, n_Absorbers, n_Channels, n_TC_CellPressures ), &
              STAT = Allocate_Status )
    IF ( Allocate_Status /= 0 ) THEN
      Error_Status = FAILURE
    ...
    TauCoeff%n_TC_CellPressures   = n_TC_CellPressures 
    TauCoeff%n_Ref_CellPressures   =  n_Ref_CellPressures
    ...
    TauCoeff%TC_CellPressure      = FP_INIT
    TauCoeff%Ref_Time= FP_INIT
    TauCoeff%Ref_CellPressure     = FP_INIT
    TauCoeff%C                 = FP_INIT
  ...
  END FUNCTION Allocate_TauCoeff_ssu
...
!
! CALLING SEQUENCE:
!       Error_Status = Assign_TauCoeff_ssu( TauCoeff_in,              &  ! Input
!                                       TauCoeff_out,             &  ! Output
!                                       RCS_Id      = RCS_Id,     &  ! Revision control
!                                       Message_Log = Message_Log )  ! Error messaging
!
...
!------------------------------------------------------------------------------
  ...
  FUNCTION Assign_TauCoeff_ssu( TauCoeff_in,   &  ! Input
                            TauCoeff_out,  &  ! Output
                            RCS_Id,        &  ! Revision control
                            Message_Log )  &  ! Error messaging
                          RESULT( Error_Status )
    ! Arguments
    TYPE(TauCoeff_ssu_type),    INTENT(IN)     :: TauCoeff_in
    TYPE(TauCoeff_ssu_type),    INTENT(IN OUT) :: TauCoeff_out
    CHARACTER(*), OPTIONAL, INTENT(OUT)    :: RCS_Id
    CHARACTER(*), OPTIONAL, INTENT(IN)     :: Message_Log
    ! Function result
    INTEGER :: Error_Status
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'Assign_TauCoeff_ssu'

    ! Set up
    IF ( PRESENT( RCS_Id ) ) RCS_Id = MODULE_RCS_ID

    ! ALL *input* pointers must be associated
    IF ( .NOT. Associated_TauCoeff_ssu(TauCoeff_In) ) THEN
      Error_Status = FAILURE
      CALL Display_Message( ROUTINE_NAME,    &
    ...
    Error_Status = Allocate_TauCoeff_ssu( TauCoeff_in%n_Orders    , &
                                      TauCoeff_in%n_Predictors, &
                                      TauCoeff_in%n_Absorbers , &
                                      TauCoeff_in%n_Channels , &
                                      TauCoeff_in%n_TC_CellPressures , &
                                      TauCoeff_in%n_Ref_CellPressures , &
                                      TauCoeff_out, &
                                      Message_Log = Message_Log )
    IF ( Error_Status /= SUCCESS ) THEN
      CALL Display_Message( ROUTINE_NAME,    &
    ...
    TauCoeff_out%TC_CellPressure      = TauCoeff_in%TC_CellPressure
    TauCoeff_out%Ref_Time= TauCoeff_in%Ref_Time
    TauCoeff_out%Ref_CellPressure     = TauCoeff_in%Ref_CellPressure
    TauCoeff_out%C                 = TauCoeff_in%C

  END FUNCTION Assign_TauCoeff_ssu


!------------------------------------------------------------------------------
!
! NAME:
!       sConcatenate_Channel_TauCoeff
!
! PURPOSE:
...
!
! CALLING SEQUENCE:
!       Error_Status = sConcatenate_Channel_TauCoeff( TauCoeff1,                &  ! Input/Output
!                                                    TauCoeff2,                &  ! Input
!                                                    RCS_Id      = RCS_Id,     &  ! Revision control
!                                                    Message_Log = Message_Log )  ! Error messaging
!
...
!------------------------------------------------------------------------------
  ...
  FUNCTION sConcatenate_Channel_TauCoeff( TauCoeff1,     &  ! Input/Output
                                         TauCoeff2,     &  ! Input
                                         RCS_Id,        &  ! Revision control
                                         Message_Log )  &  ! Error messaging
                                       RESULT( Error_Status )
    ! Arguments
    TYPE(TauCoeff_ssu_type),    INTENT(IN OUT)  :: TauCoeff1
    TYPE(TauCoeff_ssu_type),    INTENT(IN)      :: TauCoeff2
    CHARACTER(*), OPTIONAL, INTENT(OUT)     :: RCS_Id
    CHARACTER(*), OPTIONAL, INTENT(IN)      :: Message_Log
    ! Function result
    INTEGER :: Error_Status
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'sConcatenate_Channel_TauCoeff'
    ! Local variables
    INTEGER :: n_Channels, l1, l2
    TYPE(TauCoeff_ssu_type) :: TauCoeff_Tmp
    ...
    ! Check structures
    IF ( .NOT. Associated_TauCoeff_ssu( TauCoeff1 ) ) THEN
      Error_Status = FAILURE
      CALL Display_Message( ROUTINE_NAME,    &
      ...
      RETURN
    END IF
    IF ( .NOT. Associated_TauCoeff_ssu( TauCoeff2 ) ) THEN
      Error_Status = FAILURE
      CALL Display_Message( ROUTINE_NAME,    &
    ...
    IF ( TauCoeff1%n_Orders     /= TauCoeff2%n_Orders     .OR. &
         ...
         TauCoeff1%n_TC_CellPressures /= TauCoeff2%n_TC_CellPressures .OR. &
         TauCoeff1%n_Ref_CellPressures /= TauCoeff2%n_Ref_CellPressures .OR. &
    ...
    Error_Status = Assign_TauCoeff_ssu( TauCoeff1, TauCoeff_Tmp, &
                                    Message_Log = Message_Log )
    IF ( Error_Status /= SUCCESS ) THEN
      CALL Display_Message( ROUTINE_NAME, &
    ...
    ! ... now destroy it ...
    Error_Status = Destroy_TauCoeff_ssu( TauCoeff1, &
                                     Message_Log = Message_Log )
    IF ( Error_Status /= SUCCESS ) THEN
      CALL Display_Message( ROUTINE_NAME, &
    ...
    Error_Status = Allocate_TauCoeff_ssu( TauCoeff_Tmp%n_Orders, &
                                      TauCoeff_Tmp%n_Predictors, &
                                      TauCoeff_Tmp%n_Absorbers, &
                                      n_Channels, &
                                      TauCoeff_Tmp%n_TC_CellPressures, &
                                      TauCoeff_Tmp%n_Ref_CellPressures, &
                                      TauCoeff1, &
                                      Message_Log = Message_Log )
    IF ( Error_Status /= SUCCESS ) THEN
      CALL Display_Message( ROUTINE_NAME, &
    ...
    TauCoeff1%TC_CellPressure    = TauCoeff_Tmp%TC_CellPressure
    TauCoeff1%Ref_Time   = TauCoeff_Tmp%Ref_Time
    ...
    TauCoeff1%Order_Index(:,:,l1:l2,:)     = TauCoeff_Tmp%Order_Index(:,:,l1:l2,:)
    TauCoeff1%Predictor_Index(:,:,l1:l2,:) = TauCoeff_Tmp%Predictor_Index(:,:,l1:l2,:)
    TauCoeff1%C(:,:,:,l1:l2,:)             = TauCoeff_Tmp%C(:,:,:,l1:l2,:)
    TauCoeff1%Ref_CellPressure(:,l1:l2)       = TauCoeff_Tmp%Ref_CellPressure(:,l1:l2)
    ...
    TauCoeff1%Order_Index(:,:,l1:l2,:)     = TauCoeff2%Order_Index(:,:,l1:l2,:)
    TauCoeff1%Predictor_Index(:,:,l1:l2,:) = TauCoeff2%Predictor_Index(:,:,l1:l2,:)
    TauCoeff1%C(:,:,:,l1:l2,:)             = TauCoeff2%C(:,:,:,l1:l2,:)
    TauCoeff1%Ref_CellPressure(:,l1:l2)       = TauCoeff_Tmp%Ref_CellPressure(:,l1:l2)

    ! Count the number of sensors
    CALL Count_TauCoeff_Sensors_ssu( TauCoeff1 )

    ! Destroy the temporary structure
    Error_Status = Destroy_TauCoeff_ssu( TauCoeff_Tmp, &
                                     Message_Log = Message_Log )
    IF ( Error_Status /= SUCCESS ) THEN
  ...
  END FUNCTION sConcatenate_Channel_TauCoeff


!------------------------------------------------------------------------------
!
! NAME:
!       sConcatenate_Absorber_TauCoeff
!
! PURPOSE:
!       Function to concatenate two valid TauCoeff structures along
...
!
! CALLING SEQUENCE:
!       Error_Status = sConcatenate_Absorber_TauCoeff( TauCoeff1,                &  ! Input/Output
!                                                     TauCoeff2,                &  ! Input
!                                                     RCS_Id      = RCS_Id,     &  ! Revision control
!                                                     Message_Log = Message_Log )  ! Error messaging
!
...
!------------------------------------------------------------------------------
  ...
  FUNCTION sConcatenate_Absorber_TauCoeff( TauCoeff1,     &  ! Input/Output
                                          TauCoeff2,     &  ! Input
                                          RCS_Id,        &  ! Revision control
                                          Message_Log )  &  ! Error messaging
                                        RESULT( Error_Status )
    ...
    INTEGER :: Error_Status
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'sConcatenate_Absorber_TauCoeff'
    ! Local variables
    ...
    TauCoeff1%Order_Index(:,j1:j2,:,:)     = TauCoeff_Tmp%Order_Index(:,j1:j2,:,:)
    TauCoeff1%Predictor_Index(:,j1:j2,:,:) = TauCoeff_Tmp%Predictor_Index(:,j1:j2,:,:)
    TauCoeff1%C(:,:,j1:j2,:,:)             = TauCoeff_Tmp%C(:,:,j1:j2,:,:)
    ...
    TauCoeff1%Order_Index(:,j1:j2,:,:)     = TauCoeff2%Order_Index(:,j1:j2,:,:)
    TauCoeff1%Predictor_Index(:,j1:j2,:,:) = TauCoeff2%Predictor_Index(:,j1:j2,:,:)
    TauCoeff1%C(:,:,j1:j2,:,:)             = TauCoeff2%C(:,:,j1:j2,:,:)
  ...
  END FUNCTION sConcatenate_Absorber_TauCoeff
...
!
! CALLING SEQUENCE:
!       Error_Status = Equal_TauCoeff_ssu( TauCoeff_LHS,             &  ! Input
!                                      TauCoeff_RHS,             &  ! Input
...
!------------------------------------------------------------------------------
  ...
  FUNCTION Equal_TauCoeff_ssu( TauCoeff_LHS, &  ! Input
                           TauCoeff_RHS, &  ! Input
                         ...
                         RESULT( Error_Status )
    ! Arguments
    TYPE(TauCoeff_ssu_type),    INTENT(IN)  :: TauCoeff_LHS
    TYPE(TauCoeff_ssu_type),    INTENT(IN)  :: TauCoeff_RHS
    ...
    INTEGER :: Error_Status
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'Equal_TauCoeff_ssu'
    ! Local variables
    CHARACTER(256)  :: message
    INTEGER :: ULP
    LOGICAL :: Check_Once
    ...
    IF ( .NOT. Associated_TauCoeff_ssu( TauCoeff_LHS ) ) THEN
      Error_Status = FAILURE
      CALL Display_Message( ROUTINE_NAME, &
      ...
      RETURN
    END IF
    IF ( .NOT. Associated_TauCoeff_ssu( TauCoeff_RHS ) ) THEN
      Error_Status = FAILURE
      CALL Display_Message( ROUTINE_NAME,    &
    ...
    IF ( TauCoeff_LHS%n_Orders     /= TauCoeff_RHS%n_Orders     .OR. &
         ...
         TauCoeff_LHS%n_TC_CellPressures  /= TauCoeff_RHS%n_TC_CellPressures  .OR. &
         TauCoeff_LHS%n_Ref_CellPressures  /= TauCoeff_RHS%n_Ref_CellPressures  .OR. &
    ...
    jl_Channel_Loop: DO l = 1, TauCoeff_RHS%n_Channels
      jl_Absorber_Loop: DO j = 1, TauCoeff_RHS%n_Absorbers
        ...
        k_CO2_Cell_Loop: DO k = 1, TauCoeff_RHS%n_TC_CellPressures
        Compare = Compare_Float( TauCoeff_LHS%C(:,:,j,l,k), &
                                 TauCoeff_RHS%C(:,:,j,l,k), &
                                 ULP = ULP                )
        END DO k_CO2_Cell_Loop 
  ...
  END FUNCTION Equal_TauCoeff_ssu


!----------------------------------------------------------------------------------
!
! NAME:
!       Check_TauCoeff_Release_ssu
!
! PURPOSE:
!       Function to check the TauCoeff Release value.
!
! CALLING SEQUENCE:
!       Error_Status = Check_TauCoeff_Release_ssu( TauCoeff,                 &  ! Input
!                                              RCS_Id      = RCS_Id,     &  ! Revision control
!                                              Message_Log = Message_Log )  ! Error messaging
!
! INPUT ARGUMENTS:
...
!----------------------------------------------------------------------------------
  ...
  FUNCTION Check_TauCoeff_Release_ssu( TauCoeff,     &  ! Input
                                   RCS_Id,       &  ! Revision control
                                   Message_Log ) &  ! Error messaging
                                 RESULT( Error_Status )
    ! Arguments
    ...
    INTEGER :: Error_Status
    ! Local parameters
    CHARACTER(*), PARAMETER :: ROUTINE_NAME = 'Check_TauCoeff_Release_ssu'
    ! Local variables
    CHARACTER(256) :: Message
    ...
    IF ( TauCoeff%Release < TC_SSU_RELEASE ) THEN
      Error_Status = FAILURE
      WRITE( Message, '( "A TauCoeff data update is needed. ", &
                        &"TauCoeff release is ", i2, &
                        &". Valid release is ",i2,"." )' ) &
                      TauCoeff%Release, TC_SSU_RELEASE
      CALL Display_Message( ROUTINE_NAME, &
                            TRIM( Message ), &
                            Error_Status, &
                            Message_Log = Message_Log )
  ...
  END FUNCTION Check_TauCoeff_Release_ssu


!------------------------------------------------------------------------------
!
! NAME:
!       Count_TauCoeff_Sensors_ssu
!
! PURPOSE:
...
!
! CALLING SEQUENCE:
!       CALL Count_TauCoeff_Sensors_ssu( TauCoeff,       &  ! In/Output
!                                    RCS_Id = RCS_Id )  ! Optional output
!
! INPUT ARGUMENTS:
...
!------------------------------------------------------------------------------
  ...
  SUBROUTINE Count_TauCoeff_Sensors_ssu( TauCoeff, &  ! In/Output
                                     RCS_Id    )  ! Revision control
    ! Arguments
    TYPE(TauCoeff_ssu_type),    INTENT(IN OUT) :: TauCoeff
    ! Revision control
  ...
  END SUBROUTINE Count_TauCoeff_Sensors_ssu
...
!
! CALLING SEQUENCE:
!       CALL Info_TauCoeff_ssu( TauCoeff,       &  ! Input
!                           Info,           &  ! Output
!                           RCS_Id = RCS_Id )  ! Revision control
!
! INPUT ARGUMENTS:
...
!------------------------------------------------------------------------------
  ...
  SUBROUTINE Info_TauCoeff_ssu( TauCoeff, &  ! Input
                            Info,     &  ! Output
                            RCS_Id    )  ! Revision control
    ! Arguments
    TYPE(TauCoeff_ssu_type),    INTENT(IN)  :: TauCoeff
  ...
  END SUBROUTINE Info_TauCoeff_ssu

END MODULE TauCoeff_Define_ssu
