...
MODULE CRTM_IRlandCoeff
  ...
  ! ------------------------------------------------
  TYPE(IRlandCoeff_type), SAVE :: IRlandC
...
!------------------------------------------------------------------------------
  ...
  RESULT( err_stat )
    ...
    ! Get the IRlandCoeff data file type
    err_stat = IRlandCoeff_InquireFile( Filename, datatype_name )
    IF ( err_stat /= SUCCESS ) THEN
      msg = 'Error inquiring IRlandCoeff file '//TRIM(IRlandCoeff_File)//&
            ' for dataset type'//TRIM(pid_msg)
      CALL Display_Message( ROUTINE_NAME, msg, err_stat ); RETURN
    END IF


    ! Create an instance of the IRlandCoeff object
    CALL IRlandCoeff_Create( IRlandC )
        
    
    ! Load the IRlandCoeff data files based on dataset type
    SELECT CASE (TRIM(datatype_name))
      ...
      CASE (LSECATEGORY_DATATYPE)
        ...
        CALL IRlandCoeff_SetValue( IRlandC, LSEcategory = LSEcategory )

      ! Atlas dataset type
      CASE (LSEATLAS_DATATYPE)
        ! ...Read the LSEatlas file
        err_stat = LSEatlas_ReadFile( &
                     LSEatlas, &
                     IRlandCoeff_File, &
                     Quiet = .NOT. noisy )
        IF ( err_stat /= SUCCESS ) THEN
          msg = 'Error reading IRlandCoeff LSEatlas file '//TRIM(IRlandCoeff_File)//TRIM(pid_msg)
          CALL Load_Cleanup(); RETURN
        END IF
        ! ...Load the data into the shared data structure
        CALL IRlandCoeff_SetValue( IRlandC, LSEatlas = LSEatlas )
      
      ! Unknown dataset type
      CASE DEFAULT
        err_stat = FAILURE
        msg = 'File '//TRIM(IRlandCoeff_File)//' contains unrecognised datatype: '//&
              TRIM(Datatype_Name)//TRIM(pid_msg)
        CALL Load_Cleanup(); RETURN
    END SELECT
   ...
   CONTAINS
     ...
     SUBROUTINE Load_CleanUp()
       CALL IRlandCoeff_Destroy( IRlandC )
       err_stat = FAILURE
...
!------------------------------------------------------------------------------
  ...
  FUNCTION CRTM_IRlandCoeff_Destroy( Process_ID ) RESULT( err_stat )
    ...
    CALL IRlandCoeff_Destroy( IRlandC )
    IF ( IRlandCoeff_Associated( IRlandC ) ) THEN
      err_stat = FAILURE
...
!------------------------------------------------------------------------------
  ...
  FUNCTION CRTM_IRlandCoeff_IsLoaded() RESULT( IsLoaded )
    LOGICAL :: IsLoaded
    IsLoaded = IRlandCoeff_Associated( IRlandC )
