...
usage()
{
  echo " Usage: run_BeCoeff_NC2BIN.sh -l|b[fh]"
  echo
  echo "   Convert any netCDF format BeCoeff files in the current"
  echo "   directory to Binary format. File with the suffix *BeCoeff.nc"
  echo "   are converted into *BeCoeff.bin files."
  echo
...
EXE_FILE="BeCoeff_NC2BIN"
LOG_FILE="${EXE_FILE}.${ENDIAN_TYPE}_endian.log"



# Process netCDF BeCoeff files
for NC_FILE in `ls *BeCoeff.nc`; do
