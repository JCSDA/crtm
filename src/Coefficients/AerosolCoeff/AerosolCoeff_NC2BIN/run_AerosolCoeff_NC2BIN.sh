...
usage()
{
  echo " Usage: run_AerosolCoeff_NC2BIN.sh -l|b[fh]"
  echo
  echo "   Convert any netCDF format AerosolCoeff files in the current"
  echo "   directory to Binary format. File with the suffix *AerosolCoeff.nc"
  echo "   are converted into *AerosolCoeff.bin files."
  echo
...
EXE_FILE="AerosolCoeff_NC2BIN"
LOG_FILE="${EXE_FILE}.${ENDIAN_TYPE}_endian.log"



# Process netCDF AerosolCoeff files
for NC_FILE in `ls *AerosolCoeff.nc`; do
