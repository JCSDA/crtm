...
usage()
{
  echo " Usage: run_EmisCoeff_NC2BIN.sh -l|b[fh]"
  echo
  echo "   Convert any netCDF format EmisCoeff files in the current"
  echo "   directory to Binary format. File with the suffix *.EmisCoeff.nc"
  echo "   are converted into *.EmisCoeff.bin files."
  echo
...
# Process netCDF EmisCoeff files
for NC_FILE in `ls *.EmisCoeff.nc`; do
