!
! Create_IRlandSEcategory
!
! Program to create the land emissivity/reflectance coefficient files
! for the different surface type categories.
!
!
! CREATION HISTORY:
...
PROGRAM Create_IRlandSEcategory
  ...
  ! Parameters
  ! ----------
  CHARACTER(*), PARAMETER :: PROGRAM_NAME = 'Create_IRlandSEcategory'
  ...
  CALL Program_Message( PROGRAM_NAME, &
                        'Program to generate the SEcategory land '//&
...
CONTAINS
  ...
  SUBROUTINE Write_File(lsec)
    TYPE(SEcategory_type), INTENT(IN) :: lsec
    TYPE(SEcategory_type) :: lsec_copy
    ...
    err_stat = SEcategory_WriteFile( &
                 lsec, &
                 TRIM(class_name)//'.IRlandSEcategory.bin', &
    ...
    ! Clean up
    CALL SEcategory_Destroy( lsec_copy )
...
END PROGRAM Create_IRlandSEcategory
