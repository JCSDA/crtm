...
usage()
{
  echo " Usage: run_ACCoeff_NC2BIN.sh -l|b[fh]"
  echo
  echo "   Convert any netCDF format ACCoeff files in the current"
  echo "   directory to Binary format. File with the suffix *ACCoeff.nc"
  echo "   are converted into *ACCoeff.bin files."
  echo
...
EXE_FILE="ACCoeff_NC2BIN"
LOG_FILE="${EXE_FILE}.${ENDIAN_TYPE}_endian.log"



# Process netCDF ACCoeff files
for NC_FILE in `ls *ACCoeff.nc`; do
