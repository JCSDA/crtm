...
;
; NAME:
;       vanVleck_Weisskopf_SRF
;
; PURPOSE:
;       Function to calculate a Spectral Response Function (SRF) using the
;       van Vleck-Weisskopf shape.
;
; CATEGORY:
;       Spectral
;
...
;
; CALLING SEQUENCE:
;       result = vanVleck_Weisskopf_SRF( Central_Frequency, $     ; Input
;                                        Bandwidth, $             ; Input
;                                        Power, $                 ; Input
;                                        SRF_Frequency, $         ; Input
;                                        SRF, $                   ; Output
...
;
; PROCEDURE:
;       The van Vleck-Weisskopf SRF is calculated using:
;
;                 1      x     [         g                      g            ]
;         SRF = ----- . ---- . [ -------------------- + -------------------- ]
;                !PI     x0    [ ( x - x0 )^2 + g^2     ( x + x0 )^2 + g^2   ]
;
;       where g is the halfwidth,
...
FUNCTION vanVleck_Weisskopf_SRF, Input_Central_Frequency, $       ; Input
  ...
  SRF_Max_md = Width^Power
  SRF_Max_pd = ( TWO * Central_Frequency^Power ) + Width^Power

  SRF_Max = k * ( ( ONE / SRF_Max_md ) + ( ONE / SRF_Max_pd ) )
  ...
  Frequency_Sum        =      SRF_Frequency + Central_Frequency

  SRF_md = Frequency_Difference^Power + Width^Power
  SRF_pd = Frequency_Sum^Power        + Width^Power

  SRF = k * ( SRF_Frequency/Central_Frequency ) * ( ( ONE/SRF_md ) + ( ONE/SRF_pd ) )
  ...
  IF ( KEYWORD_SET( plot ) ) THEN BEGIN
    ...
    PLOT, SRF_Frequency, SRF, $
          TITLE = 'van Vleck-Weisskopf SRF, n = ' + STRING( Power, '( f3.1 )' ), $
...
; $Id$
;
; $Date: 2004/04/16 16:32:10 $
;
; $Revision$
;
; $State: Exp $
;
; $Log: vanvleck_weisskopf_srf.pro,v $
; Revision 1.1  2004/04/16 16:32:10  paulv
; Initial checkin.
;
;
;
