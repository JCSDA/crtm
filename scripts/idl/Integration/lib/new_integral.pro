...
FUNCTION New_Integral, $
  ...
  ; ...Divide up the interval between points
  nc = N_ELEMENTS(c)
  dx = DOUBLE(nc-1L)
  xidx = DINDGEN(nc)/dx
  ; ...Initialise arrays for returning interpolation results
  x_Int = _x[0]
  y_Int = _y[0]
  ; ...Loop over points sets
  Sum = ZERO
  FOR i = 0L, n-2L DO BEGIN
    xi = xidx*(_x[i+1L] - _x[i]) + _x[i]
    
    CASE Int_Type OF
      1: yi = INTERPOL(_y, _x, xi, /QUADRATIC)
      2: yi = INTERPOL(_y, _x, xi)
      3: yi = SPLINE(_x, _y, xi, 10.0d0, DOUBLE=_Double)
      4: yi = AvgQuad_Interpolate(_x, _y, xi, DOUBLE=_Double)
      ELSE: yi = SPL_INTERP(_x, _y, yd2, xi)
    ENDCASE

    x_Int = [x_Int, xi]
    y_Int = [y_Int, yi]

;print, value_locate(_x,xi)
    h = (_x[i+1L] - _x[i])/dx
    Sum = Sum + (c0 * h * (TRANSPOSE(c)#yi))
  ENDFOR
  
  ; Removed repeated points
  idx = UNIQ(x_Int, SORT(x_Int))
  x_Int = x_Int[idx]
  y_Int = y_Int[idx]
