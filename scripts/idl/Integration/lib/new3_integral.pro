...
;  ENDIF
  ...
  ; Build the interpolation index array
  ; ...Create empty array
  n_Int = (n - 1L) * (_n_Points - 1L) + 1L
  x_Int = DBLARR(n_Int)
  ; ...Segment index
  xidx = DINDGEN(_n_Points)/DOUBLE(_n_Points - 1L)
  ; ...Build up array
  FOR i = 0L, n-2L DO BEGIN
    i1 = i * (_n_Points - 1L)
    i2 = i1 + (_n_Points - 1L)
    x_Int[i1:i2] = xidx*(_x[i+1L] - _x[i]) + _x[i]
  ENDFOR
  ...
  FOR i = 0L, n-2L DO BEGIN
    i1 = i * (_n_Points - 1L)
    i2 = i1 + (_n_Points - 1L)
    h = (_x[i+1L] - _x[i])/DOUBLE(_n_Points-1L)
    Sum = Sum + (c0 * h * (TRANSPOSE(c)#y_Int[i1:i2]))
  ENDFOR
