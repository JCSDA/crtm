;+
PRO Diag_PlotTimeSeries, RadDiag_Stats_File                     , $  ; Input file 1
...
;-
  ...
  FOR n = 0, RadDiag_Stats.nVariables - 1 DO BEGIN
    ...
    ; Get limits
    minY = MIN(RadDiag_Stats.time_Data[n,Channel-1,*])
    maxY = MAX(RadDiag_Stats.time_Data[n,Channel-1,*])
    IF ( d2_Present EQ 1 ) THEN BEGIN
      minY = minY < MIN(RadDiag_Stats2.time_Data[n,Channel-1,*])
      maxY = maxY > MAX(RadDiag_Stats2.time_Data[n,Channel-1,*])
    ENDIF

    ; Plot the data
    PLOT, xJD, RadDiag_Stats.time_Data[n,Channel-1,*], $
          YRANGE=[minY,maxY], $
    ...
    OPLOT, xJD, RadDiag_Stats.time_Data[n,Channel-1,*], COLOR=DCOLOR

    IF ( d2_Present EQ 1 ) THEN BEGIN
      OPLOT, xJD2, RadDiag_Stats2.time_Data[n,Channel-1,*], $
             COLOR=D2COLOR
    ENDIF
  ...
  ; Get limits
  minY = MIN(RadDiag_Stats.time_nSamples[Channel-1,*])
  maxY = MAX(RadDiag_Stats.time_nSamples[Channel-1,*])
  IF ( d2_Present EQ 1 ) THEN BEGIN
    minY = minY < MIN(RadDiag_Stats2.time_nSamples[Channel-1,*])
    maxY = maxY > MAX(RadDiag_Stats2.time_nSamples[Channel-1,*])
  ENDIF

    ; Plot the data
  PLOT, xJD, RadDiag_Stats.time_nSamples[Channel-1,*], $
        YRANGE=[minY,maxY], $
  ...
  OPLOT, xJD, RadDiag_Stats.time_nSamples[Channel-1,*], COLOR=DCOLOR

  IF ( d2_Present EQ 1 ) THEN BEGIN
    OPLOT, xJD2, RadDiag_Stats2.time_nSamples[Channel-1,*], $
           COLOR=D2COLOR
  ENDIF

  ; Plot the difference if possible
  IF ( d2_Present EQ 1 ) THEN BEGIN
    dSamples=RadDiag_Stats.time_nSamples[Channel-1,*] - RadDiag_Stats2.time_nSamples[Channel-1,*]
    ...
    PLOT, xJD, dSamples, $
          ...
          FONT=Font, $
          /NODATA
    OPLOT, xJD, dSamples, COLOR=ECOLOR
    OPLOT, !X.CRANGE,[0,0]
  ENDIF
