...
;-
  ...
  FOR n = 0, RadDiag_Stats.nVariables - 1 DO BEGIN
    ...
    ; Get limits
    minY = MIN(RadDiag_Stats.scan_Data[n,Channel-1,*])
    maxY = MAX(RadDiag_Stats.scan_Data[n,Channel-1,*])
    IF ( d2_Present EQ 1 ) THEN BEGIN
      minY = minY < MIN(RadDiag_Stats2.scan_Data[n,Channel-1,*])
      maxY = maxY > MAX(RadDiag_Stats2.scan_Data[n,Channel-1,*])
    ENDIF

    ; Plot the data
    PLOT, RadDiag_Stats.FOV, RadDiag_Stats.scan_Data[n,Channel-1,*], $
          YRANGE=[minY,maxY], $
    ...
    OPLOT, RadDiag_Stats.FOV, RadDiag_Stats.scan_Data[n,Channel-1,*], COLOR=DCOLOR

    IF ( d2_Present EQ 1 ) THEN BEGIN
      OPLOT, RadDiag_Stats2.FOV, RadDiag_Stats2.scan_Data[n,Channel-1,*], $
