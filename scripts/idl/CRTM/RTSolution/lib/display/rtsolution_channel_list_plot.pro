PRO RTSolution_Channel_List_Plot, $
  channel_list, $  ; Input
  ...
  ; ...Do nothing if the input is not a list
  IF ( ~ ISA(channel_list,'List') ) THEN RETURN
  ; ...Throw an error is all list members are NOT RTSolution objects
  FOREACH element, channel_list DO BEGIN
    IF ( ~ ISA(element,'RTSolution') ) THEN $
      MESSAGE, 'Non-RTSolution object found in Channel_List.', $
