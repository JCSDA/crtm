...
;
; PURPOSE:
;       Function to calculate the derivative of the Planck temperature
;       (brightness temperature) with respect to radiance given the
;       wavenumber or wavelength, and radiance.  Calculation is done
;       in double precision.
;
; CALLING SEQUENCE:
;       result = Planck_dTdB( X                    , $  ; Input
;                             Radiance             , $  ; Input
;                             dTdB                 , $  ; Output
...
;
; OUTPUTS:
;       dTdB:          The computed derivative of the Planck temperature with
;                      respect to radiance.
;                      UNITS:      (K.m2.sr.cm-1)/mW  [DEFAULT]
;                                  (K.m2.sr.um)/W     [if WAVELENGTH keyword is set]
...
;
;       Calculate dT/dB in (K.m2.sr.cm-1)/mW:
;
;         IDL> Result = Planck_dTdB( 950.0, radiance, dtdb )
;         IDL> HELP, dtdb
;         DTDB            DOUBLE    =       0.60111208
;
;
...
;
;       Calculate dT/dB in (K.m2.sr.um)/W:
;
;         IDL> Result = Planck_dTdB( 11.0, radiance, dtdb, /wavelength )
;         IDL> HELP, dtdb
;         DTDB            DOUBLE    =        7.0957471
;
;
; CREATION HISTORY:
;       Written by:     Paul van Delst, CIMSS/SSEC 15-Nov-2000
;                       paul.vandelst@noaa.gov
;
;-

FUNCTION Planck_dTdB, Input_X              , $  ; Input
  ...
  ; Calculate Planck temperature derivative wrt radiance
  ; ----------------------------------------------------
  ; Get Radiance in terms of W
  r = r / R_SCALE[Unit_Index]
  ; Common term in dTdB formulation
  Argument = ( Fk1 / r ) + ONE
  ; Calculate dT/dB in K..../W
  dTdB = Fk1 * Fk2 / ( Argument * ( r * ALOG(Argument) )^2 )
  ; Convert dT/dB units to required (K...cm-1/mW or K...um/W)
  dTdB = dTdB / R_SCALE[Unit_Index]
