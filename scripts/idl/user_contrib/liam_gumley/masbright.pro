function masbright, r, band, micron = micron

;+
; Purpose:
;    Compute brightness temperature for a MAS IR band.
;
; Usage:
;    RESULT = MASBRIGHT( R, BAND )
;
; Input:
...
;
; Output:
;    RESULT  Brightness temperature (Kelvin)
;-

;- check input

loc = where( r le 0.0, count )
if count ge 1 then r( loc ) = 1.0e-7

if band lt 26 or band gt 50 then $
  message, 'MAS IR band number must be in the range 26-50'
...
if keyword_set( micron ) then begin
  ...
  return, ( bright_m( 1.0e4 / cwn( band - 26 ), r ) - ti( band - 26 ) ) / $
    ts( band - 26 )

endif else begin

  return, ( brite_m( cwn( band - 26 ), r ) - ti( band - 26 ) ) / $
    ts( band - 26 )
