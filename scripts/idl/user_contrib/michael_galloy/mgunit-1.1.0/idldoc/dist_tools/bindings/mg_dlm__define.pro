...
pro mg_dlm::addInclude, name, system=system, $
  ...
  if (keyword_set(system)) then begin
    ...
    if (~found) then begin
      if (n_elements(headerDir) gt 0L) then self.includeDirs->add, headerDir
      if (n_elements(libDir) gt 0L) then self.libDirs->add, libDir, /extract
...
pro mg_dlm::addRoutineFromPrototype, proto
  ...
  for i = 0L, n_elements(params) - 1L do begin
    param_type = mg_parse_cdeclaration(params[i], pointer=pointer, array=array)
    if (param_type ne 0) then begin
      r->addParameter, type=param_type, pointer=pointer, array=array, prototype=params[i]
    endif
  endfor
...
pro mg_dlm::addRoutinesFromHeaderFile, filename
  ...
  foreach p, prototypes do begin
    if (p ne '') then self->addRoutineFromPrototype, p
  endforeach
end
