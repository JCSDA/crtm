  ...
  if (n_elements(self.parameters) gt 0L) then begin
    foreach p, self.parameters, i do begin
      output += string(mg_newline(), $
                       p.array ? 'ARRAY' : 'SCALAR', $
                       i, $
                       format='(%"%s  IDL_ENSURE_%s(argv[%d])")')
      output += string(mg_newline(), $
                       i, $
                       mg_idltype(p.type, /type), $
                       format='(%"%s  MG_ENSURE_TYPE(argv[%d], %s)")')
...
;    
;-
pro mg_routinebinding::addParameter, type=type, prototype=prototype, $
                                     pointer=pointer, array=array                                     
  compile_opt strictarr
  
  self.parameters->add, {type: type, pointer: keyword_set(pointer), array: keyword_set(array) }
