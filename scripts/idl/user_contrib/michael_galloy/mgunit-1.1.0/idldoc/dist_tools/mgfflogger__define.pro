...
;         '(%"%s %s: %s: %s")'
;
;       where the arguments will be `systime()`, `level`, `routine`, and the 
;       log message
...
; Set properties.
;-
pro mgfflogger::getProperty, level=level, format=format, name=name, $
                             output=output
  compile_opt strictarr
...
pro mgfflogger::print, msg, level=level, from_mg_log=fromMgLog
  ...
  if (level le self->_getLevel()) then begin
    ...
    level = strupcase(self.levelNames[level - 1L])
    printf, self.lun, systime(), level, routine, msg, format=self.format
  endif
end
