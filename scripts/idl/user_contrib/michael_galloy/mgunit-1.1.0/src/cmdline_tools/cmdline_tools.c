...
#define M_MG_WRONG_TYPE       0  
  {  "M_MG_WRONG_TYPE",   "%NThe input array must be of type float." }, 
  };
...
IDL_VPTR IDL_mg_termlines(int argc, IDL_VPTR *argv) {                                 
  return IDL_GettmpLong(IDL_FileTermLines());
}


IDL_VPTR IDL_mg_termcolumns(int argc, IDL_VPTR *argv) {                                 
  return IDL_GettmpLong(IDL_FileTermColumns());
}


IDL_VPTR IDL_mg_termistty(int argc, IDL_VPTR *argv) {                                 
  return IDL_GettmpLong(MG_FileTermIsTty());
}


IDL_VPTR IDL_mg_heapid(int argc, IDL_VPTR *argv) {
  IDL_VPTR x = argv[0];
  IDL_ENSURE_SCALAR(x);
  
  if (x->type != IDL_TYP_PTR && x->type != IDL_TYP_OBJREF) {
    IDL_MessageFromBlock(msg_block, M_MG_WRONG_TYPE, IDL_MSG_LONGJMP);
  }
  
  return IDL_GettmpULong(x->value.hvid);
}


static void IDL_mg_print(int argc, IDL_VPTR *argv, char *argk) {
  int nargs;
  char *format, *cformat;
